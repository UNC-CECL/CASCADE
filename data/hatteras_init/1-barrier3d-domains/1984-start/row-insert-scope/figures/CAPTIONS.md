...
As the seaward plan view, for the rows placed BEHIND NC-12: the true-scale band hangs off the landward edge of the 1984 road (its centreline offset 10 m landward, the road being 20 m wide) and runs landward by N × 10 m, red where rows are added behind the road and blue where backbarrier rows are removed there. Box shading, dune lines and NC-12 as before; domains without a model road (GIS 1–5, 8) keep the seaward band. The strip from the dune crest through the road is untouched in this placement, and the model keeps today's setback. 29 domains gain 73 rows and 23 lose 47; 38 are unchanged. N = trunc(median paired shift / 10 m), so a row appears only once a full cell of change is measured; the largest are +7 at GIS 80 and -5 at GIS 63.
...
The copy fill for the rows placed behind NC-12, at one domain. (a) The interior as extracted (v2), 50 alongshore cells across, cross-shore rows down the page from interior row 0, shaded by elevation class (m above MHW); the model's two roadway rows as a dark band labelled NC-12, and the source window, the N rows immediately landward of the insert point, dashed in red. (b) The same interior with the N-row block inserted directly behind the road and filled with a cell-by-cell copy of the source window; the block outlined in red. Rows landward of the block are pushed down the page by N; the road does not move. The only seam is at the block's landward end, where its last row (a copy) meets the original first row. GIS 5 has no model road and keeps the seaward placement, so its window starts one row behind the crest row and the crest row itself is pushed landward by N. Numbers per domain in `fill_copy_by_domain.csv`.
