...
Written by `HAT_plot_duneline_offset.py` (`write_captions`) from the same table the figures draw from. The figures carry no in-image titles or footnotes on purpose; use these under them.
...
The two dune lines on the 2009-2014-1996 DEM at true scale on three reaches: 17-21 (the quietest reach on the island); 62-68 (1984 line landward of 1997); 78-85 (1984 line seaward of 1997). Equal aspect; each panel is cropped to 300 m either side of the local 1984 line rather than the full 2000 m domain box. Boxes are Barrier3D domains (2000 × 500 m); the number beside each is its median offset, positive where 1984 lies seaward. Elevation in m NAVD88.
...
The 2009-2014-1996 DEM with both dune lines and the 90 Barrier3D domain boxes (2000 × 500 m), in three panels of thirty domains at one common scale, south at left. A locator: at this scale the two lines are one line nearly everywhere, and the offset is read from the other figures. Both lines are clipped to the domain footprint for drawing only. Per-domain median offset (1984 minus 1997, seaward positive), from `duneline_offset_by_domain.csv`: island median +1.2 m, range -58.9 to +70.2 m; 52 of 90 domains differ by at least one 10 m Barrier3D cell, 47 of 90 are positive.
