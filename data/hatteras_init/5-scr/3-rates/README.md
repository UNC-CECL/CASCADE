...
|---|---|---|---|---|---|
| `coastsat/lrr` | per transect, the OLS slope of CoastSat position against date over the calendar window (1 Jan start to 31 Dec end, ≥ 3 positions); per domain the mean | m/yr | 1984_2004, 1996_2010, 2004_2024, 2010_2024; 1996_2024 (context only); `ext/` for the Pea Island reach | `3-rates/coastsat/lrr/coastsat_domain_lrr.py`, `3-rates/coastsat/extension/coastsat_extension_lrr.py` | **the hindcast runner** (the scoring target), the edge solve, every rate comparison |
| `coastsat/endpoint` | per transect, the mean position within ±6 months of the END dune-line date minus the same at the START date; per domain the mean | m and m/yr | the four model windows + 1996_2024 | `3-rates/coastsat/endpoint/coastsat_endpoint.py` | `net_change_vs_duneline.py` |
...
| `duneline/endpoint` | per 100 m transect, the start line's distance from the offshore datum minus the end line's; per domain the mean | m and m/yr | the four model windows + 1996_2024 | `3-rates/duneline/duneline_endpoint.py` | `HAT_rate_windows.py`, the dune edge solve, `net_change_vs_duneline.py` |
