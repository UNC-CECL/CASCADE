# 2010 island offsets (duneline), v2

Built 2026-09-24 by `scripts/input_prep/2-brie-offset/1-produce/island_offset_hybrid.py --year 2010 --version v2 --source duneline --raw-file data/hatteras_init/2-brie-offset/2010/duneline/v1/2009_duneline_offset_raw.csv`, from the SAME raw file as `../v1/` (copied here). A fix to how the file is padded, not new source data, so the count continues (v1 -> v2).
...
- **asrun (offset / 10) runs: the buffers.** asrun divides the whole file, buffers included, so a run made before 2026-09-24 reproduces only with the build it was made from: `HAT_OFFSET_VERSION_2010=v1`.
...
`../CURRENT` = `v2` since 2026-09-24. `HAT_OFFSET_VERSION_2010` in the environment outranks it for one run.
