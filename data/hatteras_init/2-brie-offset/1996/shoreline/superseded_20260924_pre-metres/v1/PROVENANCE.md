...
> **Superseded 2026-09-24; this folder was `v1`.** The switch to offset_mode `metres` re-padded this build with the model's smooth wrap-around; that build is now `../../v1/` (numbering restarted, Hannah: "restart all the years as v1"). The real domains are identical; only the 15+15 buffer domains differ (this build's are a slope segment plus a linear bridge, clipped at 0). Runs made before 2026-09-24 in `asrun` mode used this build and record `shoreline/superseded_20260924_pre-metres/v1`; reproduce one with `HAT_OFFSET_VERSION_1996_SHORELINE=superseded_20260924_pre-metres/v1`.
