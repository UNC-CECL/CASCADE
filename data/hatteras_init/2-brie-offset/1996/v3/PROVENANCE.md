...
`offset_1996_v2_vs_v3.csv` and `.png` beside this file (PDF and caption under `supporting/`). In the fixed-datum frame a positive difference is the line moved LANDWARD.

## CURRENT

`../CURRENT` = `v3` since this build.
 `hatteras_site_config._island_offset_file(1996)` resolves it; env `HAT_OFFSET_VERSION_1996` outranks the file for one run.

## Rebuild

```
python scripts/input_prep/2-brie-offset/1-produce/island_offset_hybrid.py --year 1996 --version v3 --raw-file data/hatteras_init/2-brie-offset/1996/v3/1997_duneline_offset_raw.csv
```
