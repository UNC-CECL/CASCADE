# 1996 island offsets — which SOURCE, then which version
...
take; `HAT_OFFSET_VERSION_1996` in the environment outranks it for one run
(a non-default source uses `HAT_OFFSET_VERSION_1996_<SOURCE>`, so overriding
...
Dune builds used to sit flat at `1996/v<n>/`, from when they were the only
