# 1984 road setback, dune-start method

`RoadSetback_1984_dunestart.csv` is the model-facing forcing: metres landward of **interior row 0** of the 1984-start extraction.
...
A setback measured from interior row 0 is only valid against the arrays it was measured on. Spending it on another topography version measures from a row that moved. Resolve the version through `scripts/hat_topo_version.py`; never pin one by hand.
