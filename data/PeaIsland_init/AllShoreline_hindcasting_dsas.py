...
# ============================================================
NPZ_FILE = r"/Users/rsahrae/PycharmProjects/CASCADE_Hannah/CASCADE/Output/PEA_1978_1997_Natural_State_V2/PEA_1978_1997_Natural_State_V2.npz"
...
RELATIVE_CHANGE = (
    False  # True = plot shoreline change relative to t=0, False = absolute position
)
...
RIGHT_BUFFER_START = 56  # first right-buffer CASCADE domain index
...
DSAS_DOMAIN_CSV = r"/Users/rsahrae/PycharmProjects/CASCADE_Hannah/CASCADE/data/PeaIsland_init/Shoreline_change/dsas_1978_1997_CLEAN.csv"
DSAS_DOMAIN_COL = "domain_id"  # GIS domain ID field in CLEAN file (1–90)
DSAS_CHANGE_COL = (
    "obs_total_change_m"  # total shoreline change column in CLEAN file (m)
)
# ============================================================
...
def plot_total_shoreline_change_over_run(
    ...
    if left_buffer_end is not None and left_buffer_end >= 0:
        ax.axvspan(domain_numbers[0] - 0.5, left_buffer_end + 0.5, alpha=0.1)
    if right_buffer_start is not None and right_buffer_start < ndom:
        ax.axvspan(right_buffer_start - 0.5, domain_numbers[-1] + 0.5, alpha=0.1)
    ...
    gis_ticks = np.arange(80, 120, 5)
    top_ax.set_xticks(gis_ticks)
    top_ax.set_xlabel("GIS domain number (80-120)")
    ...
    if left_buffer_end is not None and right_buffer_start is not None:
        ...
        ax.text(mid_real, y_top * 0.9, "Real island", ha="center", va="top", fontsize=9)
        ax.text(
...
def main():
    ...
    if DSAS_DOMAIN_CSV is not None:
        ...
        mask = (obs_domains_all > LEFT_BUFFER_END) & (
            obs_domains_all < RIGHT_BUFFER_START
        )
