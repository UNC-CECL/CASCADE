...
def plot_domains(casc_model, iB3D, save_dir, newfolder, buffer, units):
    ...
    for t in range(int(casc_model.barrier3d[iB3D].TMAX) - 1):
        ...
        plt.ioff()  # do not open windows when creating the plots
        ...
        ax1.text(
            0.5, 0.01, 'Ocean', c="white", horizontalalignment='center',
            verticalalignment='bottom', transform=ax1.transAxes)
        ax1.text(
            0.5, 0.98, 'Marsh/Bay', c="white", horizontalalignment='center',
...
def plot_domains_with_marsh_transect(casc_model, iB3D, save_dir, newfolder, buffer, units):
    ...
    # for t in range(5):
        ...
        # add the transect
        BB_transect_m1 = np.flip(
            casc_model.barrierbmft[iB3D].bmftc_BB.elevation[casc_model.barrierbmft[iB3D].bmftc_BB.startyear + t - 1,
            int(
                casc_model.barrierbmft[iB3D].bmftc_BB.Marsh_edge[
                    casc_model.barrierbmft[iB3D].bmftc_BB.startyear + t]):])
        if casc_model.barrierbmft[iB3D].x_b_TS_ML[t] < 0:
            ML_transect_m1 = np.append(
                np.ones([abs(int(casc_model.barrierbmft[iB3D].x_b_TS_ML[t]))]) *
                casc_model.barrierbmft[iB3D].bmftc_ML.elevation[
                    casc_model.barrierbmft[iB3D].bmftc_ML.startyear + t - 1, 1],
                casc_model.barrierbmft[iB3D].bmftc_ML.elevation[casc_model.barrierbmft[iB3D].bmftc_ML.startyear + t - 1,
                :])
        elif casc_model.barrierbmft[iB3D].x_b_TS_ML[t] > 0:
            ML_transect_m1 = casc_model.barrierbmft[iB3D].bmftc_ML.elevation[
                             casc_model.barrierbmft[iB3D].bmftc_ML.startyear + t - 1,
                             int(casc_model.barrierbmft[iB3D].x_b_TS_ML[t]):]

        whole_transect_m1 = np.append(BB_transect_m1, ML_transect_m1)

        # convert to MHW
        conversion = (casc_model.barrierbmft[iB3D].bmftc_BB.msl[
                                                         casc_model.barrierbmft[iB3D].bmftc_BB.startyear + t - 1] +
                                                     casc_model.barrierbmft[
                                                         iB3D].bmftc_BB.amp)  # Make marsh elevation relative to MHW datum
        whole_transect_mhw = whole_transect_m1 - conversion

        # Extract marsh & forest locations to plot
        x_forest_m1 = [casc_model.barrierbmft[iB3D].bmftc_BB.B - int(
            casc_model.barrierbmft[iB3D].bmftc_BB.Forest_edge[casc_model.barrierbmft[iB3D].bmftc_BB.startyear + t]),
                       len(BB_transect_m1) - int(casc_model.barrierbmft[iB3D].x_b_TS_ML[t]) + int(
                           casc_model.barrierbmft[iB3D].bmftc_ML.Forest_edge[
                               casc_model.barrierbmft[iB3D].bmftc_ML.startyear + t])]
        x_marsh_m1 = [len(BB_transect_m1) - 1,
                      len(BB_transect_m1) - int(casc_model.barrierbmft[iB3D].x_b_TS_ML[t]) + int(
                          casc_model.barrierbmft[iB3D].bmftc_ML.Marsh_edge[
                              casc_model.barrierbmft[iB3D].bmftc_ML.startyear + t])]
        y_forest_m1 = whole_transect_m1[x_forest_m1]
        y_marsh_m1 = whole_transect_m1[x_marsh_m1]

        # convert to MHW
        # x-position has not changed, just the value of y
        y_forest_mhw = whole_transect_mhw[x_forest_m1]
        y_marsh_mhw = whole_transect_mhw[x_marsh_m1]
        ...
        ax2.plot(whole_transect_mhw, c="black")
        ax2.scatter(x_forest_m1, y_forest_mhw, c="green")
        ax2.scatter(x_marsh_m1, y_marsh_mhw, c="brown")
        ax2.set_ylim(-3.1, 3)
        if t == 0:
            xlims = ax2.get_xlim()
        ax2.set_xlim(xlims)
        ...
        # add text and set title
        ax2.text(
            0.01, 0.05, 'Ocean', c="grey", horizontalalignment='left',
            verticalalignment='center', transform=ax2.transAxes)
        ax2.text(
            0.99, 0.05, 'Mainland', c="grey", horizontalalignment='right',
            verticalalignment='center', transform=ax2.transAxes)
