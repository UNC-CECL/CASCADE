...
# 3/16/2025 updated to include CASCADE Elevation plots
# these ones hold the frame constant to better observe migration patterns
# it has been updated from its original form in cascade/tools/plotters.py
...
def plot_ElevAnimation_CASCADE(
    ...
    Updated so that beach width represented here is the outwasher beach width
    """
    ...
    #             if meters_on:
    #                 locs, _ = plt.yticks()
    #                 plt.yticks(locs, locs / 10)
    #                 locs, _ = plt.xticks()
    #                 plt.xticks(locs[0:-1], locs[0:-1] / 10)
    #                 plt.xlabel("alongshore distance (m)")
    #                 plt.ylabel("cross-shore distance (m)")
    #                 ax.yaxis.set_minor_locator(AutoMinorLocator())
    ...
    for t in range(TMAX_SIM):
        ...
        for iB3D in range(ny):
            ...
            if beach_management_ny[iB3D]:
                ...
                # CHANGED FOR THE OUTWASHER BEACH RATHER THAN THE CASCADE BEACH
                beach_width = outwash[iB3D]._beach_width[t] / 10
        ...
        cbar.set_label("elevation (m MHW)", rotation=270, labelpad=25)
        plt.xlabel("alongshore distance (dam)")
        plt.ylabel("cross-shore distance (dam)")
        ...
        if km_on:
            ...
            plt.ylabel("cross-shore distance (km)")
            ax.yaxis.set_minor_locator(AutoMinorLocator())
            # ax.xaxis.set_major_formatter('{x:.f}')
            # ax.yaxis.set_major_formatter('{x:.f}')
        if meters_on:
            # locs, _ = plt.yticks()
            # plt.yticks(locs, locs * 10)
            # locs, _ = plt.xticks()
            ...
            plt.xlabel("alongshore distance (m)")
            plt.ylabel("cross-shore distance (m)")
            xtick_max = np.shape(AnimateDomain)[1]  # n_cols = x
            x_ticks = np.array(range(0, xtick_max, 10))
            x_tick_labels = x_ticks * 10
            ytick_max = np.shape(AnimateDomain)[0]  # n_rows = y
            y_ticks = np.array(range(0, ytick_max, 50))
            y_tick_labels = y_ticks * 10
            plt.xticks(x_ticks, x_tick_labels)
            plt.yticks(y_ticks, y_tick_labels)
        if y_lim is not None:
...
def plot_Elev_CASCADE_subplots(
    cascade,
    directory,
    TMAX_MGMT,
    name,
    TMAX_SIM,
    plot_timesteps,  # array with the timesteps you want to plot
    main_plot_title,  # title of the main plot (string)
    ny=1,
    beach_management_ny=None,  # list of bool the length of ny, or None for all False
    roadway_management_ny=None,
    y_lim=(150, 250),
    ...
    # establish the whole figure (not subplots yet)
    plot_num = 1
    n_cols = len(plot_timesteps)
    if fig_size is not None:
        elevFig2 = plt.figure(figsize=fig_size)
    else:
    ...
    if km_on:
        ...
        # elevFig2.supylabel("cross-shore distance (km)")
        x_text = "alongshore distance (km)"
        y_text = "cross-shore distance (km)"
    if meters_on:
        x_text = "alongshore distance (m)"
        y_text = "cross-shore distance (m)"
    else:
    ...
    for t in plot_timesteps:
        ...
        ax = elevFig2.add_subplot(1, n_cols, plot_num)
        cax = ax.pcolormesh(
            AnimateDomain,
            cmap="terrain",
            vmin=z_bot,
        ...
        # we only want the color bar for the last plot
        # we also want no y-axis labels or ticks, but we do want x-axis ones
        if plot_num == len(plot_timesteps):
            cbar = elevFig2.colorbar(cax)
            cbar.set_label("elevation (m MHW)", rotation=270, labelpad=10)
            if km_on:
                locs, _ = plt.xticks()
                plt.xticks(locs[0:-1], locs[0:-1] / 100)
            if meters_on:
        ...
        # we only want the cross-shore (y-axis) ticks for the first label, not the rest
        # but we also want xticks for this lable
        elif plot_num == 1:
            if km_on:
                locs, _ = plt.yticks()
                plt.yticks(locs, locs / 100)
    ...
    plt.subplots_adjust(top=0.8, bottom=0.2, left=0.06, right=0.95)
    # plt.subplots_adjust(left=0.05, bottom=0.1, right=0.06, top=0.15, wspace=-0.5)

    # once we make the entire plot, save it
    if fig_eps:
