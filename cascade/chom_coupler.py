...
This module couples Barrier3D with the Coastal Home Ownership Model (CHOM), an
agent-based model for coastal real estate markets.
...
Notes
---------
The user specifies the number of communities to be described by the Barrier3D models,
and the initialization of the coupler determines the (sequential) Barrier3d indices
that are associated with each CHOM model instance. For example, if 6 Barrier3D models
are used to describe the alongshore domain (each 500 m long), and 2 communities are
specified, then indices [0,1,2] and [3,4,5] are associated with each community.
...
def create_communities(ny, number_of_communities):
    ...
    if len(community_index) < number_of_communities:
        ...
        print(
            "Number of communities simulated in CHOM updated based on number of "
            "Barrier3D domains"
        )
...
def community_initial_statistics(
    ...
    for iB3D in community_indices:
        ...
        # m
        initial_beach_width = int(barrier3d[iB3D].BermEl / barrier3d[iB3D]._beta) * 10
        avg_beach_width.append(initial_beach_width)

        # maximum height of each (alongshore) row in DuneDomain (the largest foredune
        # height), then average
        ...
        # community length is the Barrier3D discretization * # of Barrier3D domains
...
def community_update_statistics(
    ...
    for iB3D in community_indices:
        # Barrier3D in dam MHW --> convert to m MSL for CHOM; b/c time_index in B3D
        # is updated at the end of the time loop, time_index-1 is the current
        # time step for passing variable to CHOM
        ...
        # nourishments.beach_width not updated until we call nourishments (after
        # chom_coupler), so just calculate here
        beach_width = (
...
class ChomCoupler:
    ...
    def __init__(
        ...
        dune_design_elevation: list of floats
            Elevation for rebuilding dunes [m NAVD88]; one value for each Barrier3D
            domain
    ...
    def update(self, barrier3d, nourishments, community_break):
        ...
        # calculate physical variables needed to update CHOM for each Barrier3D
        # model, then group by community
        for iCommunity in range(self._number_of_communities):
            community_indices = self._community_index[iCommunity]
            ...
            # update CHOM model variables before advancing one time step (but only
            # if the community hasn't been abandoned)
            if any(community_break[community_indices[0] : community_indices[-1] + 1]):
                ...
                # NOTE: dune design ele cannot yet be updated at each time step in
                # CHOM -- potential future update

                self._chom[iCommunity].update()

                # specify for each community, and iB3D cell, if it is a nourishment
                # and/or dune rebuilding year
                ...
                # this is in m^3 in CHOM, divide by the length of the community to
                # get m^3/m
