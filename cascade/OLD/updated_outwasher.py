...
# -------------------------------------------------------------------------------------------------------------------
# this file has all the updates from outwasher_testing_edges, but using b3d domain
# this includes the updated sediment flux
...
def outwasher(b3d, storm_series, runID):
    ...
    interior_domain = np.flip(b3d.InteriorDomain, axis=0)
    ...
    if numstorm > 0:
        ...
        for n in range(numstorm):
            ...
            if n == 0:
                ...
                # full_domain = full_domain[7:, :]  # testing removing the bay
            ...
            # this is only used to calculate C, but we are going to just set a value for that and not worry about Si
            # Si = np.mean((interior_domain[-1, :] - interior_domain[0, :]) / len(interior_domain))
            ...
            ax4.set_title("Cross shore elevation profile\n "
                          "shoreface slope = {0}".format(round(m_shoreface, 3)))
            ax4.legend()
            plt.show()
...
sound_data = [s+0.2 for s in sound_data]  # [dam MHW] just increasing the values
# used 0.2 when bay was included
# used 0.05 other times
...
b3d = Barrier3d.from_yaml("C:/Users/Lexi/PycharmProjects/Barrier3d/tests/test_params/")
runID = "72_C_ss_sedout_edges_b3d"
