...
def compare_bay_height_to_dune_gap_height(
    ...
    we will either compare the bay level to all the dune lines or just the
    first dune line

    :param dune_flow_type: string, "full" or "first" to specify rules for
        initializing flow routing through the dunes
    ...
    :return: continue_flow_routing: bool, continue flow routing rules
        (True) or end this outwash storm time step (False)
    """
    ...
    if dune_flow_type.lower() == "full":
        ...
        # if the bay level exceeds the min of all the dune lines, we will continue
        # flow routing
        if counter == n_dunes:
            continue_flow_routing = True
        # if the bay level does not exceed the min of all the dune lines, we will
        # set the discharge to 0 and nothing else will happen this time step
        else:
...
def calculate_discharge_at_dune_gaps(
    ...
    :param int_width: int, interior barrier width [dam]
    :param bayhigh: float, current bay elevation [dam MHW]
    :param discharge_array: zeros array, used to store the total discharge
        (sum of Q1, Q2, Q3) for each cell [dam^3]
    :param FR_rules_array: zeros array, stores the values for each cell
        (1 for underwater, 2 for downhill, 3 for start cell)
    ...
    :return: updated discharge_array, FR_rules_array, downhill_array, and
        start_cell_array
    """
    if dune_flow_type.lower() == "full":
        ...
        for dune in range(n_dunes):
            ...
            # find and return (Dow) the index of the dune cells that are lower than the
            # bay elevation
            Dow = [
            ...
            if len(Dow) > 0:
                ...
                # check for a section of adjacent submerged dune cells acting as one
                # dune gap
                while i < (len(Dow) - 1):
                    ...
                    if adjacent == 1:
                        ...
                        stop = i
                        # calculate the average elevation of the dune gap across all its
                        # cells
                        x = elev_array[
                        ...
                        # calculate the velocity (based on gravitational flow) and flows
                        # (Q=VA)
                        overtop_vel = (
                ...
                # if none of the dune cells are overwashed (bay level too low),
                # we set all the discharge to 0 for this outwash storm time step
        ...
        if len(Dow) > 0:
            ...
            # check for a section of adjacent submerged dune cells acting as
            # one dune gap
            while i < (len(Dow) - 1):
                adjacent = Dow[i + 1] - Dow[i]
                if adjacent == 1:
                    ...
                    stop = i
                    # calculate the average elevation of the dune gap across
                    # all its cells
                    x = elev_array[
                        time_step, dune_gap_row, Dow[start] : (Dow[stop] + 1)
                    ]
                    ...
                    # find the bay level above the average dune gap elevation
                    Rexcess = bayhigh - Hmean
                    # calculate the velocity (based on gravitational flow) and
                    # flows (Q=VA)
                    overtop_vel = math.sqrt(2 * 9.8 * (Rexcess * 10)) / 10  # (dam/s)
                    overtop_flow = overtop_vel * Rexcess * 3600  # (dam^3/hr)
                    # assign the flow to each cell in the dune gap
...
def check_underwater(
    ...
    """
    check for hydraulic connection from the bay to the dune gaps: there must be a
    path of submerged cells
    ...
    :param FR_rules_array: stores the values for each cell (1 for underwater,
        2 for downhill, 3 for start cell)
    to indicate flow routing initialization
    :param downhill_array: builds on the underwater array,
    indicates the cells that are both underwater and downhill
    :param start_cell_array: builds on the underwater and downhill array,
    indicates the cells that are underwater, downhill, and start cells for flow routing
    :return: bay_sufficient (boolean indicating if there is a hydraulic connection
        from the bay to the dunes)
    :return: updated FR_rules_array, downhill_array, and start_cell_array
    """
    cont = 1
    current_row = start_row
    # backtrack from the dunes to the bay (0th row) to see if there are
    # underwater cells connected to the dunes here we start at the dune
    # gaps and look upstream (one cell diagonally left, center, and diagonally right)
    while current_row >= 1 and cont == 1:
        ...
        # get the left and right indices of the submerged cells (hydraulically
        # connected to our submerged cell)
        for i in range(len(gaps)):
            gap_index.append(gaps[i][0])
            gap_index_left.append(gaps[i][0] - 1)
            gap_index_right.append(gaps[i][0] + 1)

        # in case our gap exists at the first or last column, remove the left
        # and right cells from the list
        ...
        # create one list of all the UNIQUE indices that are connected to
        # our submerged cells
...
def clear_floating_submerged_cells(
    ...
    """
    remove any "floating" submerged cells. these are submerged cells that
    started at a dune gap, but did not reach the bay. the previous function,
    check_underwater, only checks if there is at least 1 dune gap that is
    hydraulically connected to the bay; it does not check every dune gap. this
    function clears any submerged cells that are not connected to the bay
    ...
    """
    # starting at the second row, check to make sure all bay cells touch a
    # previous bay cell, if not, set cell to 0
    for row in range(1, width):
        ...
        for g in submerged_cells_index:
            if g == 0:
                ...
                # in middle columns, check diagonal left, center and diagonal
                # right cells
                if (
...
def calculate_slopes(
    ...
    :param length: int, alongshore barrier length [dam]
    :param time_step: int, current time step of the outwash storm
    :param s1_vals: zeros array, used to store the S1 slope values for each time
        step, row, and column
    :param s2_vals: zeros array, used to store the S2 slope values for each time
        step, row, and column
    :param s3_vals: zeros array, used to store the S3 slope values for each time
        step, row, and column
    :param sL_left_vals: zeros array, used to store the lateral slopes coming
        from the left cell into the center cell
    :param sL_right_vals: zeros array, used to store the lateral slopes coming
        from the right cell into the center cell
    :return updated (nonzero) s1_vals, s2_vals, s3_vals, sL_left_vals, and
        sL_right_vals arrays
    """
        ...
        for col in range(length):
            ...
            if row != width - 1:
                if col > 0:  # col = 0 means there are no cols to the left
                    ...
                    # -999 is greater than the max slope, so no sediment will
                    # move that direction
                    S1 = -999
                ...
                if col < (
                    ...
                    # prevent the domain from losing sediment on its sides
                    # -999 is greater than the max slope, so no sediment will move
                    # that direction
                    S3 = -999
            ...
            # where sL is positive (downhill) into the center cell from the left or
            # right
            if col == 0:
                ...
                # there are no cells to the right, so sediment cannot come from the
                # right
...
def initialize_discharge_at_start_cells(
    ...
    """
    starting at the most landward dune line, find the cells that have downhill slopes
    toward the dune gaps, and set the initial discharge at the selected start cells
    using a conservation of flow rule

    :param start_row: int, equal to the interior width
    :param time_step: int, current time step of the outwash storm
    :param discharge: numpy array, stores the total discharge (sum of Q1, Q2, Q3)
        for each cell [dam^3]
    ...
    :param length: int, alongshore barrier length [dam]
    :param FR_rules_array: stores the values for each cell (1 for underwater, 2
        for downhill, 3 for start cell)
    to indicate flow routing initialization (EQUAL TO DOWNHILL ARRAY HERE)
    :param start_cell_array: builds on the underwater and downhill array,
    indicates the cells that are underwater, downhill, and start cells for flow routing
    :param int_width: int, interior barrier width [dam]
    :param init_dis: zeros array, stores the initial discharge values after
        specifying the start cells used for plotting purposes only, can be deleted
    ...
    while current_row >= 1 and cont == 1:
        ...
        # in case our cell exists at the first or last column, remove the left
        # and right cells from the list
        ...
        s2_vals = []
        s3_vals = []
        # since we only want downhill or zero slope values, the minimum slope
        # allowable is 0
        min_slope = 0

        # get the previously calculated S1, S2, and S3 values for the arrays
        # (left, right, and regular index) since we are looking upstream from
        # the current cell, the S1 value comes from the right index and the S3 value
        ...
        if new_total_cells > 0:
            ...
            # if we do not, then stop backtracking and move onto locating the
            # start cells for flow routing
            cont = 0

    # find the starting cells: the most landward, downhill cells in each column
    # not hydraulically connected to a more landward, downhill cell
    ...
    for l in range(length):
        if (
            ...
            start_cell_col = l
            # if we are at the first column, just check if upstream diag right
            # column is downhill
            if (
                start_cell_col == 0
            ...
            # if we are at the last column, just check if upstream diag left
            # column is downhill
            elif (
    ...
    if start_cells > 0:
        # apply conservation of flow from the discharge expected at the dune
        # gaps to the start cells
...
def flow_routing_with_lateral_transport(
    ...
    :param n_time_steps: int, total length of outwash storm (original
        hydrograph length * substep)
    :param width: int, cross-shore barrier width [dam]
    :param int_width: int, interior barrier width [dam]
    :param length: int, alongshore barrier length [dam]
    :param elevation: numpy array of elevations for each model time step
    :param storm_series: numpy array with years of outwash events in the first
        column and the bay hydrograph in the second column
    :param n_dune_rows: int, number of dune rows
    :param dune_flow_dynamics: string, "full" or "first" to specify rules for
        initializing flow routing through the dunes
    :param nn: float = 0.5, flow parameter
    :param max_slope: float, max uphill slope value that still allows sediment transport
    :param OW_TS: array that stores the outwasher time steps that produce an
        outwash event
    ...
    :param qs_lost_total: float, tracks the sediment leaving the barrier from
        the bottom of the domain [dam^3]
    :return: updated elevation (domain), qs_lost_total, Discharge,
        elev_change_array, underwater_array, downhill_array,
    ...
    # next three arrays are separate solely for plotting purposes, future
    # versions only actually need 1
    ...
    for TS in range(n_time_steps):
        ...
        if continue_flow_routing is False:
            ...
            # initialize the flow routing array based on all the dunes or just
            # the first (most landward) row
            ...
            # we check to see if the dune gaps line up through the entire dune
            # line, if not, we do not consider it a dune gap
            if (
                dune_flow_dynamics.lower() == "full"
            ):  # we will not route flow through the dunes
                for col in range(length):
                    # we check all the rows in each column and if there is a
                    # discharge value = 0 in any column position, we set the
                    # entire column to 0
                    if np.any(
                        Discharge[TS, int_width : (int_width + n_dune_rows), col] == 0
                    ):
                        Discharge[TS, int_width : (int_width + n_dune_rows), col] = 0
                        # we also update our arrays as they will no longer be
                        # submerged at these locations
                        underwater_array[
            ...
            if bay_sufficient is True:
                ...
                # set the first (most landward) row of dunes as downhill (2 in
                # our arrays)
                for col in range(length):
                ...
                # since we assume that bed slopes drive water slopes, we want
                # to focus the flow on the downhill cells look for downhill cells
                # connected to the dune gaps. then, we select the most landward
                # cells not influenced by any adjacent cells as the start cells
                # and initalize discharge
                ...
                # if there is not a hydraulic connection from the bay to the
                # dune gaps, set discharge to 0
                Discharge[TS] = 0
            ...
            if len(start_flow_row) > 0:
                ...
                for d in range(start_flow_route, width):
                    for i in range(length):
                        ...
                        if Discharge[TS, d, i] > 0:
                            ...
                            # UPDATE DISCHARGE
                            # discharge is defined for the next row, so we do not need
                            # to include the last row and the first row of discharge
                            # was already defined
                            if d != width - 1:
                                # Cell 1
                                if i > 0:
                            ...
                            # if the Qs_out of the current cell will make that cell
                            # a lower elevation than 3 m, we will only move the amount
                            # of sediment up to -3 m (adjust Qs_out)
                            limit = -0.3
                            if (
                            ...
                            # track the sediment being added to a cell from its
                            # neighbors (QsL_total) we also must track the sediment
                            # leaving the neighbor cells (SedFluxOut)
                ...
                # calculate and save volume of sediment leaving the island
                # for every hour
                qs_lost_total = (
...
class Outwasher:
    ...
    def __init__(
        ...
        :param bay_depth: float, depth of bay behind island segment, currently
            set to 0.3 [dam MHW]
        ...
        :param percent_washout_to_shoreface: int, percent of outwashed sediment
            that will go to the shoreface
        :param outwash_beach_file: string, npy file for beach [dam MHW]
        :param initial_beach_width: int, intial beach width for shoreline
            dynamics (separate from outwash beach file)
        """
        ...
        # full or first, specifies how we evaluate the bay level compared to the
        # dune elevation. If full, we will require the bay elevation to exceed
        # all dune rows (the dune gap must extend through all the dune lines)
        # if first, we will only require the bay elevation to exceed the first
        ...
        self._full_dunes = []
        self._full_domain = []
        self._post_outwash_full_domain_TS = np.zeros(time_step_count, dtype=object)
    ...
    def update(self, b3d):
        ...
        # beach width reaches zero, turn dune migration in B3D back on (occurs in beach_width_dune_dynamics function)
        ...
        if self._time_index - 1 in self._outwash_storms[:, 0]:
            ...
            # save post-storm dune, interior, shoreline, shoreface params before
            # outwash mods (a 0.5 yr time step)
            ...
            # (run the same bay level 100 times with proportionally reduced sed
            # transport)
            if self._substep != 1:
            ...
            if self._outwash_beach is None:
                ...
                for s in range(len(beachface_domain)):
                    # the first row of the beach face depends on the last row of
                    # the beach
                    if s == 0:
                        beachface_domain[s, :] = (
            ...
            # the full domain of outwasher starts with the interior domain, then
            # the dune, and beach
            ...
            # update barrier3d interior and dune domain class variables for outwash
            # modifications ----------------------
            # NOTE: in barrier3d the elevation array is just the interior domain,
            # here it is the full domain
