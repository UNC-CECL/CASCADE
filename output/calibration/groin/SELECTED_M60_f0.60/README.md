...
## Refreshed 2026-09-11, and where each file comes from

Every image here is a HAND COPY — nothing builds this folder — so the set went
stale the moment the figure scripts were restyled under the project house style
(`scripts/site_layer/hat_figure_style.py`; see `CALIBRATION_FIGURES.md` beside those
scripts for what changed). All ten figures and seven of the eight gifs were
re-copied from the current renders on 2026-09-11, keeping these numbered names,
and a PDF now sits beside each PNG. The source map below makes the next refresh
a re-run rather than a reconstruction.

Two things follow from the restyle. The images no longer carry their own
titles, statistics lines or footnote paragraphs; that prose is in a
`CAPTIONS.md` beside each SOURCE render, not here, and this README is what
carries the argument. And the numbers quoted in the tables on this page were
re-checked against the new renders on refresh: figure 01's fit numbers
(15.68 → 11.65 m, 26%, best 11.54 m, 13 tied cells) and figure 03's fillet
peaks still hold.

| file here | copied from, under `output/calibration/groin/` unless noted |
|---|---|
| `figures/01_why_M60_f0.6.png` | `figures/why_M60_f06.png` |
| `figures/02_period1_fit_profiles.png` | `figures/period1_top_n_profiles.png` |
| `figures/03_full_life_1967_2017.png` | `figures/full_life_1967_2017.png` |
| `figures/04_fillet_vs_surveys_hindcast.png` | `figures/timeseries_check.png` |
| `figures/05_rig_f_bracket.png` | `figures/rig_f_bracket.png` |
| `figures/06_three_targets.png` | `figures/fig_three_targets.png` |
| `figures/07_M_f_identifiability.png` | `figures/fig_Mf_identifiability.png` |
| `figures/08_window_sensitivity_D4D7.png` | `figures/fig_d4d7_window.png` |
| `figures/09_profiles_by_M.png` | `figures/profiles_by_M/fig_all_M_profiles.png` |
| `figures/10_sediment_budget.png` | `figures/sediment_budget.png` |
| `gifs/00_full_life_vs_observations_1967_2017.gif` | `figures/full_life_gif/shoreline_D2-D12_1967_2017.gif` |
| `gifs/00b_full_life_planform_1967_2017.gif` | `output/calibration/groin_rig/HAT_1967_2018_edge_calibrated_groin/…_PLOT_shoreline_evolution.gif` — the run's own planform, drawn by the pipeline's plotting, not by a groin-sweep script; unchanged by the restyle |
| `gifs/01_CHOSEN_M60_f0.60.gif` | `figures/zoom_gifs_D2_D12/shoreline_D2-D12_M60_f0.60.gif` |
| `gifs/02_compare_M40_f0.60_too_weak.gif` | `figures/zoom_gifs_D2_D12/shoreline_D2-D12_M40_f0.60.gif` |
| `gifs/03_compare_M95_f0.60_too_strong.gif` | `figures/zoom_gifs_D2_D12/shoreline_D2-D12_M95_f0.60.gif` |
| `gifs/04_compare_M160_railed_joint_fit.gif` | `figures/zoom_gifs_D2_D12/shoreline_D2-D12_M160_f0.60.gif` |
| `gifs/05_compare_f0.00_total_failure.gif` | `figures/zoom_gifs_D2_D12/shoreline_D2-D12_M60_f0.00.gif` |
| `gifs/06_compare_f1.00_never_deteriorates.gif` | `figures/zoom_gifs_D2_D12/shoreline_D2-D12_M60_f1.00.gif` |

One fix came out of the refresh: the comparison gifs (01–06) advertised a
no-groin reference line in the legend and never drew it, because the baseline
cell on disk is `M0_be-42.6` with no `_f` suffix and only the suffixed name was
tried. The dotted line is in these frames now.
...
is kept at `output/archive/2026-08-30_groin-railed-ranking/joint_fit_RAILED_ranking_20260830.json` and each entry
