# Wave-climate sensitivity, natural scenario, offset in metres (2026-09-24)
...
`experiments/2026-09-24-island-offset-scale-wave-tuning/` (found there under
...
- On the ridge the bias is −0.2 to −0.5 m/yr and the best share explained is
  12% (Hs 1.25, high-angle 0.5; RMSE 1.10 against the flat line's 1.17): the
  model beats a flat line, but not by much.
...
**Measured 2026-09-24** (`experiments/2026-09-24-overwash-fix/NOTE.md`): the
...
| `stage2/grid_wave_height_x_high_angle_both_periods.png` | the grid as lines: share explained and bias against Hs, a line per high-angle fraction |
...
- A stage-2 figure for the abandoned Hs × Tp grid (its 12 runs are scored in
  `tables/all_runs.csv`).

## Reproduce

```
python scripts/sensitivity_analysis/natural_wave_sensitivity.py run stage1
python scripts/sensitivity_analysis/natural_wave_sensitivity.py score
python scripts/sensitivity_analysis/natural_wave_sensitivity.py run stage2
python scripts/sensitivity_analysis/natural_wave_sensitivity.py score
python scripts/sensitivity_analysis/plot_natural_wave_sensitivity.py
```
