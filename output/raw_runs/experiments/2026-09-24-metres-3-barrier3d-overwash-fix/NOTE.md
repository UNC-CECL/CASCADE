...
reorganisation. `logs/diagnostics/` holds the NUMBA_BOUNDSCHECK and JIT-off
runs on the unpatched model that located the bug (moved here from the
wave-sensitivity study, where they were run as `crash_check*`).
...
code, and the cause of every crash in `experiments/2026-09-24-metres-2-wave-sensitivity/`
...
Script: `scripts/hatteras_ms/experiments/HAT_metres_3_overwash_fix.py`. Every
launch recorded the Barrier3D branch and commit it ran on (`launches.jsonl`).

| member | twin (unpatched) |
|---|---|
| natural and full-management baselines, 1996–2010 and 2010–2024 (metres, zeroBE, no groin; Hs 1.0, Tp 8, asymmetry 0.8, high-angle 0.45) | the wave-sensitivity study's runs (`../2026-09-24-metres-2-wave-sensitivity/`), made the same day on the same code |
...
- **So no earlier conclusion changes**, ÷10 or metres. What the bug did cost is
  the crashes, which removed runs from the sweeps (10 in the wave-sensitivity
...
- The upstream issue draft:
  `barrier3d_route_overwash_issue_draft.md` in this folder
