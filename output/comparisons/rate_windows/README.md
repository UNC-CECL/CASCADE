# rate_windows - the model against an observation, four windows
...
full management, groin off. Two observations, the CoastSat waterline and the
digitised dune line, and for each the run whose two end domains were solved
on it, so the comparison is fair at GIS 1 and 90 by construction. Same layout,
same colours, one y range across every panel.

One tree since 2026-09-17. Before that this was `observed_vs_modeled_windows/`
(CoastSat, 09-15) and `duneline_vs_modeled_windows/` (the dune line, 09-16,
relaid 09-17), two scripts with two vocabularies for one axis and the dune
tree carrying four copies of its own layout (Hannah: "more organized and
clear, and potentially condensed, especially with the naming"). Nothing was
re-run; the skill numbers below are the ones the two old READMEs carried.

## Which figure answers which question

Start with `both/both_grid.png`.

| question | folder | observation | model line |
|---|---|---|---|
| Does the model follow the waterline? | `coastsat/means/` | CoastSat per-domain mean LRR, ±1 std | OLS rate, ends solved on CoastSat |
...
```
coastsat/means/            coastsat_means_<w>.png
...
sensitivity/mixed-estimator/   duneline_endpoint_olsmodel_<w>.png: the two-survey
                               observation against the model's OLS rate, the one
                               pairing that mixes estimators, kept so the choice
                               is visible
```
...
`loess` and `lrr` readings the TARGET takes the fill and a light outline, the
