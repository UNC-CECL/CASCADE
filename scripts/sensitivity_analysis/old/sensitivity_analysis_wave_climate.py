...
parameter values to understand their influence on model comparison.

Purpose:
--------
- Test wave_height, wave_period, wave_asymmetry, wave_angle_high_fraction
- Keep all other parameters constant (original, background erosion, etc.)
...
START_YEAR = 1978
RUN_YEARS = 19
STORM_FILE = os.path.join(HATTERAS_DATA_BASE, 'original', 'hindcast_storms', 'storms_1978_1997.npy')
...
DUNE_OFFSET_FILE = os.path.join(
    HATTERAS_DATA_BASE, '2-brie-offset', 'hindcast_1978_1997',
...
for i_list in range(START_REAL_INDEX, END_REAL_INDEX):
    ...
    DUNE_FILE_PATHS.append(os.path.join(HATTERAS_DATA_BASE, 'dunes', '2009_v1', f'domain_{file_num}_dune_2009.npy'))
    ELEVATION_FILE_PATHS.append(os.path.join(HATTERAS_DATA_BASE, 'topography', '2009_v1', f'domain_{file_num}_topography_2009.npy'))
...
def extract_shoreline_change_rates(cascade_output_dir):
    """
    Extract average shoreline change rates from CASCADE comparison.
    Uses Hannah's proven method from AllShoreline_with_DSAS_HAH_V2_LRR.py
    
    Args:
        cascade_output_dir (str): Path to CASCADE comparison directory
...
def run_single_cascade_simulation(wave_params, run_name):
    ...
    Returns:
        str: Path to comparison directory
    """
    print(f"\n  Running: {run_name}")
...
def create_sensitivity_plots(results_df, timestamp, observed_rates=None):
    ...
    if 'mean_change' not in results_df.columns:
        ...
        print("   Check the comparison directories to see what files were created.")
