...
SENSITIVITY_ANALYSES = {
    "wave_height": {
        ...
        "values":  [0.75, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0, 3.5, 4.0, 4.5, 5.0],
    },
    "wave_period": {
        "label":   "Wave Period",
        "units":   "s",
...
def _plot_parameter_sweep(param_name, param_cfg, rate_profiles, cs_series,
    ...
    # Model lines
    # Drowned/crashed runs (those that stopped early) produce rates in the
    # hundreds of m/yr and would blow out the y-axis.  Identify stable runs
    # first so we can set a sensible scale before drawing anything.
    STABLE_THRESHOLD = 50.0   # m/yr — anything beyond this is a drowned artifact
    stable_pvs = [
        pv for pv in rate_profiles
        if np.abs(rate_profiles[pv][START_REAL_INDEX:END_REAL_INDEX]).max() < STABLE_THRESHOLD
    ]
    drowned_pvs = [pv for pv in rate_profiles if pv not in stable_pvs]
    if drowned_pvs:
        print(f"  Note: {len(drowned_pvs)} drowned run(s) excluded from y-axis scale "
              f"(peak rate > {STABLE_THRESHOLD} m/yr): {drowned_pvs}")
    ...
    # Y-axis scale: use only stable runs + CoastSat/LOESS so drowned runs
    # don't compress the physically meaningful part of the plot.
    stable_arrays = [
        rate_profiles[pv][START_REAL_INDEX:END_REAL_INDEX] for pv in stable_pvs
    ]
    loess_arrays  = [lc["rate"] for lc in loess_curves.values()]
    cs_arrays     = [cs["rate"] for cs in cs_series] if cs_series else []
    scale_arrays  = stable_arrays + loess_arrays + cs_arrays

    if scale_arrays:
        all_vals = np.concatenate(scale_arrays)
        all_vals = all_vals[np.isfinite(all_vals)]
    else:
        all_vals = np.array([-3.0, 3.0])   # fallback
