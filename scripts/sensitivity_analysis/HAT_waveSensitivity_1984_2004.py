...
SENSITIVITY_ANALYSES = {
    "wave_height": {
        ...
        "units":   "m",
        "enabled": True,
        "values":  [0.25, 0.5, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0],  # ← extend as needed
    },
    "wave_period": {
        ...
...
def run_sweep(param_name, param_cfg, param_output_dir, cs_series):
    """
...
def _plot_parameter_sweep(param_name, param_cfg, rate_profiles, cs_series, output_dir):
    """
    ...
    # CoastSat reference lines
    data_handles = []
    for cs in cs_series:
        gis_x = cs["x"] - START_REAL_INDEX + FIRST_FILE_NUMBER
        if cs["active"]:
            ax.plot(gis_x, cs["rate"], color=CS_ACTIVE_COLOR,
                    lw=1.8, ls="-", zorder=4, label=cs["label"])
            data_handles.append(
                Line2D([0], [0], color=CS_ACTIVE_COLOR, lw=1.8, ls="-",
                       label=cs["label"])
            )
        else:
            ax.plot(gis_x, cs["rate"], color=CS_REF_COLOR,
                    lw=1.2, ls="--", alpha=0.6, zorder=3,
                    label=cs["label"] + " (ref)")
            data_handles.append(
                Line2D([0], [0], color=CS_REF_COLOR, lw=1.2, ls="--", alpha=0.6,
                       label=cs["label"] + " (ref)")
            )
