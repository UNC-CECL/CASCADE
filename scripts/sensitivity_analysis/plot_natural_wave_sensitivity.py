#!/usr/bin/env python3
r"""
plot_natural_wave_sensitivity.py -- the figures for the natural-scenario wave sensitivity
==============================================================================
Reads tables/all_runs.csv and tables/stage2_selection.csv (written by
natural_wave_sensitivity.py), each scored run's tables/shoreline_change_rate.csv,
the CoastSat LRR target and the observed CoastSat position change. Writes,
under output/raw_runs/sensitivity/2026-09-24-natural-waves/figures/:
...
import natural_wave_sensitivity as study  # noqa: E402
...
def fig_grid(t):
    sel_path = study.TABLES_DIR / "stage2_selection.csv"
    ...
    fig, axes = plt.subplots(2, 2, figsize=figsize("double", height=5.2), sharex=True,
                             constrained_layout=True)
    ...
    png = FIG / "stage2" / f"grid_{g1}_x_{g2}_both_periods.png"
    save(fig, png, dpi=300, close=True)
    pd.DataFrame(rows).to_csv(support_dir(png.parent) / f"{png.stem}.csv", index=False)
    record_caption(png, (
        f"Stage 2: {study.PARAMS[g1][2].lower()} against "
        f"{study.PARAMS[g2][2].lower()}, the two parameters whose stage-1 range moved "
        ...
        "bias. Columns: the two windows. Lines: values of the second parameter, light to "
