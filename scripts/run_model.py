...
def get_roadway_statistics(
    folder_prefix,
    natural_barrier_elev,
    natural_barrier_width,
    individual_fid=None,
    tmax=None,
):
    ...
    if individual_fid is not None:
        ...
        year_abandoned.append(cascade.roadways[0]._time_index - 1)
        drown.append(b3d.drown_break)
        road_bulldozed.append(
            sum(cascade.roadways[0]._road_overwash_volume[0:tmax_sim] > 0)
        )  # only counts true elements
        overwash_removed.append(
            sum(cascade.roadways[0]._road_overwash_volume[0:tmax_sim])
        )  # m^3
        dune_rebuilt.append(int(sum(cascade.roadways[0]._dunes_rebuilt_TS[0:tmax_sim])))
        road_relocated.append(
            int(sum(cascade.roadways[0]._road_relocated_TS[0:tmax_sim]))
        )

    else:
        for filenum in range(100):
            output = np.load(folder_prefix + filenum + ".npz", allow_pickle=True)
            cascade = output["cascade"]
            cascade = cascade[0]
            b3d = cascade.barrier3d
            ...
            dune_rebuilt.append(
                int(sum(cascade.roadways[0]._dunes_rebuilt_TS[0:tmax_sim]))
            )
            road_relocated.append(
                int(sum(cascade.roadways[0]._road_relocated_TS[0:tmax_sim]))
            )
...
# def get_nourishment_statistics(folder_path, individual_fid=None):
#
#     # year abandonded
#     # total length of simulation (yr)
#     # from above, drown? (yes, no)
#     # overwash removal -- number of times?
#     # total m^3 of overwash
#     # rebuild dune -- total number
#     # nourishments -- total number
#     # difference between natural barrier height and width at the end of the simulation
...
def cascade_1kyr_runs():
        ...
        def pt75():
            def low():
                ...
                cascade_pt75_h1m_low = RUN_6_CASCADE_noAST_Rave_SLR_pt004_Roadways(
                    ...
                    road_ele=0.8,  # average initial elevation, 0.774 m MHW
                    road_width=20,  # m
                    road_setback=20,  # m
                    dune_design_elevation=1.8,  # m MHW, rebuild to 1 m dune above the roadway
                    dune_minimum_elevation=1.3,  # m MHW, allow dune to erode down to 0.5 m above the roadway
                    storm_file="StormSeries_1kyrs_VCR_Berm1pt9m_Slope0pt04_01.npy",
                    elevation_file="b3d_pt75_1353yrs_low-elevations.csv",
                    dune_file="barrier3d-default-dunes.npy",
                    background_erosion=0.0,
                ...
                cascade_pt75_h2m_low = RUN_6_CASCADE_noAST_Rave_SLR_pt004_Roadways(
                    ...
                    road_width=20,  # m
                    road_setback=20,  # m
                    dune_design_elevation=2.8,  # m MHW, rebuild to 2 m dune above the roadway
                    dune_minimum_elevation=1.3,  # m MHW, allow dune to erode down to 0.5 m above the roadway
                    storm_file="StormSeries_1kyrs_VCR_Berm1pt9m_Slope0pt04_01.npy",
                    elevation_file="b3d_pt75_1353yrs_low-elevations.csv",
                    dune_file="barrier3d-default-dunes.npy",
                ...
                cascade_pt75_h3m_low = RUN_6_CASCADE_noAST_Rave_SLR_pt004_Roadways(
                    ...
                    road_width=20,  # m
                    road_setback=20,  # m
                    dune_design_elevation=3.8,  # m MHW, rebuild to 3 m dune above the roadway
                    dune_minimum_elevation=1.3,  # m MHW, allow dune to erode down to 0.5 m above the roadway
                    storm_file="StormSeries_1kyrs_VCR_Berm1pt9m_Slope0pt04_01.npy",
                    elevation_file="b3d_pt75_1353yrs_low-elevations.csv",
                    dune_file="barrier3d-default-dunes.npy",
    ...
    def nourishments():
        ...
        def rebuild_threshold_1m():
            def pt75_low():
                # Community reached minimum width, drowned at 177 years
            ...
            def pt75_high():
                # Community reached minimum width, drowned at 551
            ...
            def pt45_low():
                # Community reached minimum width, drowned at 412 years
                ...
                # Community reached minimum width, drowned at 301 years
            ...
            def pt45_high():
                # Community reached minimum width, drowned at 573 years; Barrier has HEIGHT DROWNED at t = 635 years
    ...
    def alongshore_variable_management():
        ...
        def nourishment_pt75_low_split_roadways_BE1m_3km():
            ...
            rmax = [0.95] * number_barrier3d_models
            elevation_file = [
                "b3d_pt75_1353yrs_low-elevations.csv"
            ] * number_barrier3d_models
            dune_file = ["barrier3d-default-dunes.npy"] * number_barrier3d_models
            storm_file = "StormSeries_1kyrs_VCR_Berm1pt9m_Slope0pt04_01.npy"
            dune_design_elevation = [2.8] * number_barrier3d_models  # 2 m scenario
            num_cores = 6  # for my laptop, max is 15
            dune_minimum_elevation = (
                1.3  # m MHW, allow dune to erode down to 0.5 m above the roadway
            )
            road_ele = 0.8  # m MHW
            road_width = 20  # m
            road_setback = 20  # m
        ...
        def nourishment_pt45_low_left_middle_pt75_low_roadways_right_pt45_low_roadways_4pt5km():
            ...
            elevation_file = (
                ...
                + ["b3d_pt75_1353yrs_low-elevations.csv"] * 3
                + ["b3d_pt45_8757yrs_low-elevations.csv"] * 3
            )
            dune_file = ["barrier3d-default-dunes.npy"] * number_barrier3d_models
            storm_file = "StormSeries_1kyrs_VCR_Berm1pt9m_Slope0pt04_01.npy"
            dune_design_elevation = [3.6] * 3 + [2.8] * 3 + [3.6] * 3
            dune_minimum_elevation = (
                [2.1] * 3 + [1.3] * 3 + [2.1] * 3
            ...
            # middle roadway drowned at 167, 167, 147 due to SLR, road cannot be below 0 m MHW -- created a curved shape
            AST_3domains_BE1m = (
                RUN_9_CASCADE_Rave_SLR_pt004_AlongshoreVariableManagement(
                    nt=200,
            ...
            # Roadway drowned in place at 91 years due to SLR - road cannot be below 0 m MHW
            # Barrier has HEIGHT DROWNED at t = 98 years
            # Barrier has HEIGHT DROWNED at t = 98 years
            ...
            # Roadway width drowned at 128 years, 20.0% of road borders water
            # Roadway width drowned at 136 years, 20.0% of road borders water
            # Roadway width drowned at 136 years, 20.0% of road borders water
            # Barrier has HEIGHT DROWNED at t = 137 years
            # Barrier has HEIGHT DROWNED at t = 137 years
            AST_3domains_AccSLR = (
                RUN_9_CASCADE_Rave_SLR_pt004_AlongshoreVariableManagement(
                    nt=200,
    ...
    def roadways():
        ...
        def pt75_low():
            ...
            (
                ...
                tmin=0,
                tmax_roadways=313,  # Roadway width drowned at 313 years, 20.0% of road borders water
                tmax_sim=1000,
                plot_name="b3d_pt75_h1m_plots_low",
                run_road_mgmt=True,
            )
            ...
            (
                ...
                tmin=0,
                tmax_roadways=182,  # Roadway width drowned at 182 years, 20.0% of road borders water
                tmax_sim=1000,
                plot_name="b3d_pt75_h2m_plots_low",
                run_road_mgmt=True,
                gif_on=False,
            ...
            (
                ...
                tmin=0,
                tmax_roadways=104,  # Island is to narrow for roadway to be relocated. Roadway eaten up by dunes at 104 years
                tmax_sim=1000,
                plot_name="b3d_pt75_h3m_plots_low",
                run_road_mgmt=True,
            )
        ...
        def pt45_low():
            ...
            (
                ...
                tmin=0,
                tmax_roadways=542,  # Island is to narrow for roadway to be relocated. Roadway eaten up by dunes at 542 years
                tmax_sim=1000,
                plot_name="b3d_pt45_h1m_plots_low",
                run_road_mgmt=True,
            )
            ...
            (
                ...
                tmin=0,
                tmax_roadways=558,  # Roadway width drowned at 558 years, 20.0% of road borders water
                tmax_sim=1000,
                plot_name="b3d_pt45_h2m_plots_low",
                run_road_mgmt=True,
            )
            ...
            (
                ...
                tmin=0,
                tmax_roadways=321,  # # Roadway width drowned at 321 years, 20.0% of road borders water
                tmax_sim=1000,
                plot_name="b3d_pt45_h3m_plots_low",
                run_road_mgmt=True,
            )
        ...
        def pt45_high():
            ...
            (
                ...
                name_prefix="6-B3D_Rave_pt45_Roadways_3mDune_20mSetback_20mWidth_high_v2",
                tmin=0,
                tmax_roadways=519,
                tmax_sim=1000,
                plot_name="b3d_pt45_h3m_plots_high",
