...
# Simulation years — used to convert DSAS annual rate → total change
START_YEAR = 1978
END_YEAR   = 1997
...
CONTROL_DOMAINS = {
    ...
    # accretion
    65: ('South of Salvo', 'accretion', +4),

    # erosion (the problem area)
    85: ('Rodanthe', 'erosion', -1.5),
...
def load_cascade_object(output_dir):
    """
    Load the single CASCADE NPZ file and return the cascade object.
    CASCADE saves everything into one file named after the run.
    """
    npz_files = [f for f in os.listdir(output_dir) if f.endswith('.npz')]
    if not npz_files:
        print(f"  ✗ No NPZ files found in {output_dir}")
        return None
    npz_path = os.path.join(output_dir, npz_files[0])
    print(f"  ✓ Loading: {npz_files[0]}")
    data = np.load(npz_path, allow_pickle=True)
    cascade = data["cascade"][0]
    print(f"  ✓ CASCADE object loaded — {len(cascade.barrier3d)} barrier3d domains")
    return cascade


def get_b3d_for_domain(cascade, gis_domain_number, num_buffer):
    """
    Return the Barrier3D object for a GIS domain number (1-indexed).
    CASCADE array index = (gis_domain - 1) + num_buffer
    """
    array_index = (gis_domain_number - 1) + num_buffer
    if array_index >= len(cascade.barrier3d):
        print(f"  ✗ Array index {array_index} out of range "
              f"(cascade has {len(cascade.barrier3d)} domains)")
        return None, array_index
    print(f"  ✓ GIS domain {gis_domain_number} → CASCADE array index {array_index}")
    return cascade.barrier3d[array_index], array_index


def extract_shoreline_variables(b3d, domain_label):
    """
    Extract all shoreline-related variables from a Barrier3D object and
    compute change with BOTH sign conventions for comparison.
    Returns a dict with findings.
    """
    results = {'label': domain_label, 'variables': {}}

    # --- x_s_TS: raw shoreline position time series ---
    if hasattr(b3d, 'x_s_TS') or hasattr(b3d, '_x_s_TS'):
        x_s_ts = np.array(b3d.x_s_TS if hasattr(b3d, 'x_s_TS') else b3d._x_s_TS)
        x_s_change = x_s_ts[-1] - x_s_ts[0]

        print(f"\n  x_s_TS (raw shoreline position, DAM):")
        print(f"    t=0  x_s: {x_s_ts[0]:.4f} dam  ({x_s_ts[0]*10:.2f} m)")
        print(f"    t=-1 x_s: {x_s_ts[-1]:.4f} dam  ({x_s_ts[-1]*10:.2f} m)")
        print(f"    Raw change (x_s[-1] - x_s[0]): {x_s_change:+.4f} dam = {x_s_change*10:+.2f} m")
        print(f"    Negated change (-1 × raw):      {-x_s_change:+.4f} dam = {-x_s_change*10:+.2f} m")
        ...
        results['variables']['x_s_TS'] = {
            ...
            'corrected_change_m': float(-x_s_change * 10),
        }
    else:
        print("  ✗ x_s_TS not found on barrier3d object")

    # --- ShorelineChangeTS: per-year discrete cell change ---
    if hasattr(b3d, 'ShorelineChangeTS') or hasattr(b3d, '_ShorelineChangeTS'):
        sc_ts = np.array(b3d.ShorelineChangeTS if hasattr(b3d, 'ShorelineChangeTS')
                         else b3d._ShorelineChangeTS)
        cumulative = float(np.sum(sc_ts))

        print(f"\n  ShorelineChangeTS (per-year cell change, DAM):")
        print(f"    Cumulative sum: {cumulative:+.4f} dam = {cumulative*10:+.2f} m")

        results['variables']['ShorelineChangeTS'] = {
            'cumulative_dam': cumulative,
            'cumulative_m': cumulative * 10,
        }
    else:
        print("  ✗ ShorelineChangeTS not found on barrier3d object")

    # --- ShorelineChange: total scalar ---
    if hasattr(b3d, 'ShorelineChange') or hasattr(b3d, '_ShorelineChange'):
        sc_total = float(b3d.ShorelineChange if hasattr(b3d, 'ShorelineChange')
                         else b3d._ShorelineChange)
        print(f"\n  ShorelineChange (total scalar, DAM): {sc_total:+.4f} dam = {sc_total*10:+.2f} m")
        ...
        print("  ✗ ShorelineChange not found on barrier3d object")
...
def run_diagnostic():
    ...
    # Load the single CASCADE NPZ once — all domains are inside it
    print("Loading CASCADE output...")
    cascade = load_cascade_object(OUTPUT_DIR)
    if cascade is None:
        return

    # Compute DSAS total change from rate × years
    run_years = END_YEAR - START_YEAR
    dsas_total_lookup = {
        dom: rate * run_years
        for dom, (_, _, rate) in CONTROL_DOMAINS.items()
    }

    all_results = {}

    for domain_num, (label, expected, dsas_rate) in CONTROL_DOMAINS.items():
        dsas_total = dsas_rate * run_years
        print(f"\n{'='*60}")
        print(f"GIS DOMAIN {domain_num}: {label}")
        print(f"  Expected: {expected}")
        print(f"  DSAS rate: {dsas_rate:+.1f} m/yr  →  total over {run_years} yr: {dsas_total:+.1f} m")
        print(f"{'='*60}")

        b3d, array_idx = get_b3d_for_domain(cascade, domain_num, NUM_BUFFER)
        if b3d is None:
            continue

        results = extract_shoreline_variables(b3d, label)
        results['expected'] = expected
        results['dsas_total_m'] = dsas_total
...
def plot_xsTS_timeseries(domain_number, run_years=19):
    ...
    if cascade is None:
        return
    b3d, _ = get_b3d_for_domain(cascade, domain_number, NUM_BUFFER)
    if b3d is None:
        return

    if not (hasattr(b3d, 'x_s_TS') or hasattr(b3d, '_x_s_TS')):
        print("x_s_TS not found on barrier3d object")
        return

    x_s = np.array(b3d.x_s_TS if hasattr(b3d, 'x_s_TS') else b3d._x_s_TS)
    ...
    if hasattr(b3d, 'ShorelineChangeTS') or hasattr(b3d, '_ShorelineChangeTS'):
        sc_ts = np.array(b3d.ShorelineChangeTS if hasattr(b3d, 'ShorelineChangeTS') else b3d._ShorelineChangeTS)
    else:
        sc_ts = np.zeros(len(x_s))
