...
# ---------------------------------------------------------------------------
# Supports 200-row topography_dunes. Filters stitched view to domains 30–134.
...
#   .../CASCADE/scripts/input_preperation/topography_dunes/topo_figures.py
PROJECT_ROOT = Path(__file__).resolve().parents[3]

# Data locations (adjust if your year subfolders differ)
BASE_PATH = PROJECT_ROOT / "data" / "hatteras_init"
TOPO_DIR  = BASE_PATH / "topography_dunes" / "2009"
...
def build_title(stitched: bool, domain_stems):
    if stitched:
        return f"Stitched island (domains {DOMAIN_MIN_ID}–{DOMAIN_MAX_ID}) — back-barrier topography_dunes, {TITLE_YEAR}"
    return f"{nice_domain_label(domain_stems[0], 1)} — back-barrier topography_dunes, {TITLE_YEAR}"
