...
# --- USER PATHS --------------------------------------------------------------
# Folder with domain_#.npy elevation arrays
LOAD_PATH       = r"C:\Users\hanna\PycharmProjects\CASCADE\data\hatteras_init\elevations\2009_pea_hatteras"
...
def process_domain_file(in_path: Path, topo_out_dir: Path, dune_out_dir: Path) -> None:
    ...
    if AUTO_ORIENT:
        ...
        print(
            f"[check] {in_path.name}: edge p25 left={left_q:.2f}, "
            f"right={right_q:.2f} -> ocean_on_right={ocean_on_right}"
        )
