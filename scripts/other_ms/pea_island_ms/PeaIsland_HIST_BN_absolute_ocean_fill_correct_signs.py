...
DUNE_OFFSET_FILE = r"/Users/rsahrae/PycharmProjects/CASCADE_Hannah/CASCADE/data/PeaIsland_init/Island_offset/Island_Dune_Offsets_1978_1997_PADDED_71.csv"

STORM_FILE = os.path.join(
    PEA_DATA_BASE,
    "Storms",
    "storms_1992_2010.npy",
)

ROAD_SETBACK_FILE = r"/Users/rsahrae/PycharmProjects/CASCADE_Hannah/CASCADE/data/PeaIsland_init/Roads/raw_offset/1978_road_setback_CLEAN.csv"
...
APPLY_AUTOMATIC_BN_THRESHOLD = True
...
def add_site_annotations(ax, sign_mode="relative"):
    """
    Add Pea Island site labels and direction arrows.

    sign_mode options:
        "relative": use for relative shoreline-change plots and rate plots.
                    Positive y = accretion; negative y = erosion.

        "absolute": use for absolute x_s plots.
                    Smaller x_s = seaward/ocean/advance.
                    Larger x_s  = landward/back-barrier/retreat.
    """
    ...
    # -------------------------------------------------------------------------
    # Direction/sign annotation.
    # -------------------------------------------------------------------------
    x_arrow = xmax - 0.035 * xrange

    if sign_mode == "absolute":
        # Absolute x_s convention:
        # larger x_s  = landward/back-barrier = retreat/erosion direction
        # smaller x_s = seaward/ocean = advance/accretion direction
        ax.annotate(
            "Landward / back-barrier\nlarger $x_s$ = retreat",
            xy=(x_arrow, ymax - 0.10 * yrange),
            xytext=(x_arrow, ymax - 0.28 * yrange),
            fontsize=9,
            color="0.20",
            ha="center",
            va="center",
        ...
        ax.annotate(
            "Seaward / ocean\nsmaller $x_s$ = Prograde",
            xy=(x_arrow, ymin + 0.10 * yrange),
            xytext=(x_arrow, ymin + 0.28 * yrange),
            fontsize=9,
            color="0.20",
            ha="center",
            va="center",
        ...
        # Relative shoreline-change and shoreline-rate convention:
...
def plot_yearly_relative_shoreline_and_bn(
    ...
    for t in range(max_nt):
        ...
        add_site_annotations(ax, sign_mode="relative")
...
def plot_yearly_absolute_shoreline_and_bn(
    ...
    """
    Yearly absolute shoreline-position plot.

    IMPORTANT SIGN CONVENTION:
        y-axis = absolute x_s position in meters.

        smaller x_s = seaward / ocean side
        larger x_s  = landward / back-barrier side

    Therefore:
        line moves DOWN through time = seaward advance / accretion
        line moves UP through time   = landward retreat / erosion

    The blue fill is drawn below the selected reference shoreline curve because
    lower x_s is the ocean/seaward direction.
    """
    yearly_dir = os.path.join(output_dir, filename_prefix)
    os.makedirs(yearly_dir, exist_ok=True)
    ...
    # -------------------------------------------------------------------------
    # Global y-limits across all years/scenarios so GIF does not jump.
    # -------------------------------------------------------------------------
    all_vals = []
    ...
    # -------------------------------------------------------------------------
    # Which curve should define ocean fill?
    # Prefer historical BN scenario because this plot is about historical BN.
    # -------------------------------------------------------------------------
    preferred_bn_label = "Roadway management + historical BN"
    ...
    ocean_fill_label = bn_curve_label
    ...
    for t in range(max_nt):
        ...
        # ---------------------------------------------------------------------
        # OCEAN FILL:
        # lower x_s is seaward/ocean, so fill from y_min up to shoreline curve.
        # ---------------------------------------------------------------------
        ocean_curve = shoreline_position_by_label[ocean_fill_label][t, :]
        ...
        ax.fill_between(
            ...
            linewidth=0,
            zorder=0,
            label="Ocean / seaward side\n(lower $x_s$)",
        )

        # Optional: lightly tint landward side to reinforce interpretation.
        ax.fill_between(
            domain_numbers,
            ocean_curve,
            y_max,
            ...
            linewidth=0,
            zorder=0,
            label="Landward / back-barrier side\n(higher $x_s$)",
        )

        # ---------------------------------------------------------------------
        # Plot shoreline-position curves.
        # ---------------------------------------------------------------------
        ...
        # Correct sign/orientation annotations for absolute x_s.
        add_site_annotations(ax, sign_mode="absolute")
        add_real_domain_top_axis(ax)

        # Clear labels inside plot.
        ax.text(
            0.012,
            0.06,
            "Ocean / seaward side\n(smaller $x_s$)",
        ...
        # ---------------------------------------------------------------------
        # Lower panel: historical BN volume.
        # ---------------------------------------------------------------------
        ax_bn.bar(
            domain_numbers,
            bn_total_m3,
        ...
        ax_bn.set_xlabel(
            "CASCADE domain index including buffers",
            fontsize=11,
            fontweight="bold",
        )
        ax_bn.set_ylim(0, max_bn_volume * 1.25)
...
def main():
    ...
    ROADWAY_BEACH_DUNE_ON = ROADWAY_MANAGEMENT_ON.copy()
    ...
    scenarios = [
        ...
        dict(
            scenario_name="roadwayMngmt_HIST_BN",
            label="Roadway management + historical BN",
            roadway_on=ROADWAY_MANAGEMENT_ON,
            beach_dune_on=ROADWAY_BEACH_DUNE_ON,
    ...
    for scenario in scenarios:
        for Hs in WAVE_HEIGHTS_TO_TEST:
            run_name = (
                ...
                f"{scenario['scenario_name']}_Hs{Hs:.1f}"
            ).replace(".", "p")

            if APPLY_AUTOMATIC_BN_THRESHOLD:
                run_name = run_name + "_thresholdBN_ON"
            else:
                run_name = run_name + "_thresholdBN_OFF"
            ...
            change_rate = total_change / float(time_span_years)
    ...
    for (scenario_label, Hs), rate in rate_profiles.items():
        ax.plot(
            domain_numbers,
            rate,
            linewidth=2.2,
            label=(
                f"{scenario_label}, Hs={Hs} m, "
                f"wave angle high fraction={FIXED_WAVE_ANGLE_HIGH_FRACTION}"
            ),
        )
    ...
    ax.axvline(START_REAL_INDEX, linestyle="--", linewidth=1.0, color="0.45", alpha=0.75)
    ax.axvline(END_REAL_INDEX - 1, linestyle="--", linewidth=1.0, color="0.45", alpha=0.75)
    ax.axhline(0.0, linestyle="--", linewidth=1.2, color="0.25", alpha=0.75)
    ...
    ax.set_title(
        ...
        f"threshold BN={APPLY_AUTOMATIC_BN_THRESHOLD} | "
    ...
    for rate in rate_profiles.values():
        finite_rate = np.asarray(rate)[np.isfinite(rate)]
        all_y_values.extend(finite_rate)

    for s in coastsat_series:
        finite_obs = np.asarray(s["rate"])[np.isfinite(s["rate"])]
        all_y_values.extend(finite_obs)
    ...
    fig.text(
        0.5,
        -0.035,
        "Positive shoreline change rate indicates accretion; negative shoreline change rate indicates erosion. "
        "Threshold BN effectiveness is saved separately in threshold_BN_effective_log.csv.",
        ha="center",
        va="center",
        fontsize=9,
        style="italic",
    ...
    fig_out = os.path.join(
        OUTPUT_BASE_DIR,
        f"PEA_{START_YEAR}_{END_YEAR}_shoreline_rates_HIST_BN_{threshold_tag}_with_CoastSat_annotated.png",
    )

    fig.savefig(fig_out, dpi=300, bbox_inches="tight")
    print(f"\nSaved annotated shoreline-rate plot: {fig_out}")
