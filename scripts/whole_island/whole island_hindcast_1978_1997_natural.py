...
- Dynamically loads 135 vertical elevation profiles (118 real + 30 buffer domains).
- The 'Natural State' configuration explicitly disables all management modules.

DOMAIN NUMBERING:
- Real domains: 1-105 (south to north)
- Total with buffers: 1-135 (15 buffer + 118 real + 15 buffer)
"""
import copy
import numpy as np
import os
...
# ==============================================================================
# Total alongshore domains:
# 15 Left Buffer + 118 Real + 15 Right Buffer = 148
TOTAL_DOMAINS = 148
NUMBER_BARRIER3D_MODELS = TOTAL_DOMAINS

# ------------------------------------------------------------------------------
# Real Domain Indexing (0-based)
# Left Buffer:  indices 0–14
# Real Domains: indices 15–132  (118 total)
# Right Buffer: indices 133–147
# ------------------------------------------------------------------------------
START_REAL_DOMAIN_INDEX = 15
END_REAL_DOMAIN_INDEX = 133       # Exclusive (15 + 118 = 133)

# ------------------------------------------------------------------------------
# File numbering for real domains (1–118)
# These correspond to your domain_1.npy ... domain_118.npy files
# ------------------------------------------------------------------------------
START_FILE_NUMBER = 1
END_FILE_NUMBER = 118
...
# If roads span all real domains, match the real-domain range.
# ------------------------------------------------------------------------------
START_ROAD_INDEX = START_REAL_DOMAIN_INDEX      # 15
END_ROAD_INDEX = END_REAL_DOMAIN_INDEX          # 133 (exclusive)
# Note: Update with specific domains if roads only exist on a subset.
...
DUNE_LOAD_ALL = os.path.join(HATTERAS_DATA_BASE, 'island_offset', '1978_whole_island', 'Whole_Island_Dune_Offsets_1978_1997_PADDED_148.csv')
...
ROAD_LOAD_FILE = os.path.join(HATTERAS_DATA_BASE, 'roads', 'offset', '1978_whole_island','CASCADE_Whole_Island_RoadSetback_1978.csv')
...
if START_YEAR == 1978:
    ...
    RUN_NAME = 'Whole_HAT_1997_2020_Natural_State'
