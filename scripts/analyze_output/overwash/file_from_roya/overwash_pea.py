...
for _ in range(START_REAL_INDEX):
    DUNE_FILE_PATHS.append(os.path.join(PEA_DATA_BASE, "Buffer", "sample_1_dune_Pea.npy"))
    ELEVATION_FILE_PATHS.append(os.path.join(PEA_DATA_BASE, "Buffer", "sample_1_topography_Pea.npy"))

for i_list in range(START_REAL_INDEX, END_REAL_INDEX):
    file_num = FIRST_FILE_NUMBER + (i_list - START_REAL_INDEX)
    DUNE_FILE_PATHS.append(os.path.join(PEA_DATA_BASE, "Dunes", "1996_dunes", f"resampled_1996_domains_{file_num}_dune.npy"))
    ELEVATION_FILE_PATHS.append(os.path.join(PEA_DATA_BASE, "Topo","2011_modified", f"resampled_2011_domains_{file_num}_topography.npy"))
...
def get_Qow_ts(b3d):
    ...
    raise AttributeError(
        ...
        "Try: print([k for k in dir(cascade.barrier3d[15]) if 'qow' in k.lower() or 'ow' in k.lower()])"
    )
...
def plot_Qow_lines_by_year(
    ...
    for i in range(0, len(years), plot_every_n_years):
        ax.plot(domain_labels, Q[i, :], lw=1.5, alpha=0.85, label=str(years[i]))
...
def plot_single_year_Qow(
    ...
    if year_to_plot not in years:
        raise ValueError(f"Year {year_to_plot} not found. Available years: {years[0]} to {years[-1]}")
