...
OUTPUT_NAME        = 'initialization_poster_1978.png'
...
plt.rcParams.update({
    ...
    'xtick.major.size': 4,
    'ytick.major.size': 4,
    'figure.facecolor': '#0d1b2a',   # deep navy background
...
# ---- Colormap ----
# Custom terrain-like colormap that pops on a dark background
from matplotlib.colors import LinearSegmentedColormap

ocean_color   = '#0d1b2a'    # blends with background for NaN/ocean
cmap_colors   = [
    (0.0,  '#1a6b9a'),   # subtidal / near-0  → steel blue
    (0.35, '#4ea8c4'),   # low intertidal
    (0.5,  '#f0e68c'),   # sea level / shoreline → sand yellow
    (0.65, '#d4a843'),   # low dune
    (0.80, '#b8860b'),   # mid dune
    (1.0,  '#ffffff'),   # dune crest → white
]
cmap = LinearSegmentedColormap.from_list(
    'hatteras', [(v, c) for v, c in cmap_colors])
...
cbar.set_label('Elevation (m MSL)', fontsize=12, color='white', labelpad=10)
cbar.ax.yaxis.set_tick_params(color='white', labelcolor='white')
cbar.outline.set_edgecolor('#4a6fa5')
cbar.set_ticks([-1, 0, 1, 2, 3, 4])

# ---- Road extent ----
road_lw = 1.8
road_color = '#ff9f43'   # warm amber — visible on dark bg
ax.axvline(road_col_start - 0.5, color=road_color, lw=road_lw, ls='--', zorder=5, alpha=0.9)
ax.axvline(road_col_end   + 0.5, color=road_color, lw=road_lw, ls='--', zorder=5, alpha=0.9)
...
# Road label at top
road_mid = (road_col_start + road_col_end) / 2
ax.text(road_mid, canvas_rows * 0.97,
        'NC-12 corridor',
        ha='center', va='top', fontsize=9.5,
        color=road_color, fontstyle='italic', fontweight='bold',
        bbox=dict(boxstyle='round,pad=0.25', facecolor='#0d1b2a',
                  edgecolor=road_color, linewidth=0.8, alpha=0.85))
...
ax.set_xlabel('Domain (S → N,  Hatteras Inlet to Oregon Inlet)', fontsize=12, labelpad=8)
...
for spine in ['bottom', 'left']:
    ax.spines[spine].set_color('#4a6fa5')
...
leg = ax.legend(handles=legend_handles, loc='lower right', fontsize=10,
                framealpha=0.7, facecolor='#1a2d45', edgecolor='#4a6fa5',
                labelcolor='white')
...
for i in range(0, NUM_REAL_DOMAINS, 10):
    ...
    ax.axvline(x, color='white', lw=0.4, alpha=0.25, zorder=2)
