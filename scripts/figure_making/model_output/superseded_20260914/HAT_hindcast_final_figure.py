...
Writes output/comparisons/hindcast_calibrated/hindcast_calibrated_full_management.png
...
# WHAT EACH PRESET IS, IN THE WORDS THE FIGURE USES. Added 2026-09-14 with the
# edgeBE companion. --preset already existed, but the output filename, the
# title and the footnote were all hardcoded to calibBE: building any other
# preset OVERWROTE the calibrated figure with a page that still called itself
# calibrated. Everything that names the configuration now comes from here.
#
# `stem` keeps calibBE on the filename the rest of the project references;
# every other preset is keyed by its own name so the two cannot collide.
PRESETS = {
    "calibBE": dict(
        stem="hindcast_calibrated",
        series="CASCADE, calibrated",
        title="calibrated — full management, groin on, calibrated source/sink",
        config="calibBE source/sink, full management (roadway + beach/dune, with "
               "nourishment in 2004-2024), groin on at the fitted M = 60 m/yr, "
               "f = 0.6. This figure answers how well the calibrated model "
               "reproduces the observed shoreline; scenario_grid_by_preset.png is "
               "where presets and management scenarios are compared.",
        interior_corrections=True),
    "edgeBE": dict(
        stem="hindcast_edgeBE",
        series="CASCADE, edgeBE (interior uncalibrated)",
        title="source/sink calibration removed — full management, groin on, "
              "edgeBE (D1/D90 edge values only)",
        config="edgeBE source/sink -- the D1 and D90 edge values ONLY, every "
               "interior correction removed. Full management (roadway + "
               "beach/dune, with nourishment in 2004-2024), groin on at the "
               "fitted M = 60 m/yr, f = 0.6. This is the calibrated figure's "
               "companion: the edge values are kept because they are solved by "
               "buffer-cell reproduction rather than fitted to a residual, so "
               "what is removed here is exactly what was calibrated. The misfit "
               "below is therefore the job the source/sink field does.",
        interior_corrections=False),
    "zeroBE": dict(
        stem="hindcast_zeroBE",
        series="CASCADE, zeroBE (no source/sink)",
        title="no source/sink field — full management, groin on, zeroBE",
        config="zeroBE -- no background-erosion field anywhere, the D1/D90 edges "
               "included. Full management (roadway + beach/dune, with "
               "nourishment in 2004-2024), groin on at the fitted M = 60 m/yr, "
               "f = 0.6. The edges carry no absorber here, so they are free to "
...
def main():
    ...
    for axis, (key, meta) in zip(axes, PERIODS.items()):
        ...
        # The grey band marks domains the CALIBRATION withheld. Under a preset
        # that corrects no interior domain it would imply a distinction the run
        # does not make, so it is not drawn.
        if vocab["interior_corrections"]:
        ...
        axis.plot(gis, m, color=meta["colour"], linewidth=2.4, marker="o",
                  markersize=4.5, zorder=5, label=vocab["series"])
    ...
    handles += [
        ...
        *([Patch(facecolor="#C8C8C8", alpha=0.30,
          ...
          if vocab["interior_corrections"] else []),
    ...
    figure.suptitle(
        f"Cape Hatteras hindcast, {vocab['title']}",
        fontsize=14, y=0.985)
    figure.tight_layout(rect=(0, 0.105, 1, 0.965))
    figure.text(
        0.01, 0.012,
        "ONE CONFIGURATION ONLY: " + vocab["config"] + " The quantity is lrr_m_yr, an OLS slope through the run's annual states, "
        "scored against a target built the same way -- the rate is what was calibrated, so it is what the model is judged on.\n"
        ...
        + (("Grey: outside the frozen zone set, diagnosed but never corrected, because zone membership was fixed before the calibration "
            "iterated rather than grown to fit. ") if vocab["interior_corrections"] else
           ("No grey band: this preset applies NO interior correction anywhere, so the frozen zone set does not divide the domains here "
            "and drawing it would imply a distinction the run does not make. ")) +
        "Purple: D1 and D90, solved separately by buffer-cell reproduction and carrying rates ~10x the interior, so they are "
        "excluded from the skill numbers rather than allowed to dominate them.",
        fontsize=7.4, color="#333333", wrap=True)

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    path = OUT_DIR / f"{vocab['stem']}_full_management.png"
