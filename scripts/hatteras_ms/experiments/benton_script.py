...
# Assumes dune raw_offset file has shape (N_domains, 2)
dune_offset_all = np.loadtxt(dune_load_name, skiprows=1, delimiter=',')

# Select the correct column for the initial dune raw_offset based on the start year
...
for i_list in range(START_REAL_DOMAIN_INDEX, END_REAL_DOMAIN_INDEX):
    ...
    # Dune Profile (2009_v1 profiles for the dune structure)
    dune_path_template = os.path.join(HATTERAS_BASE, 'dunes', '2009_v1', f'domain_{file_num}_resampled_dune_2009.npy')
    d_file.append(dune_path_template)

    # Topography Profile (2009_v1 profiles for the backbarrier)
    elev_path_template = os.path.join(HATTERAS_BASE, 'topography_dunes', '2009_v1',
...
def alongshore_connected(
    ...
    # --------- SAVE ---------
    # Define the comparison directory relative to the current script's location
