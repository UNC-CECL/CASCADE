...
Writes  output/groin_sweep/joint_fit.json    fitted (M, f, be1) per preset
        ...
        output/groin_sweep/joint_<preset>_surface.png
...
def joint_fit(preset):
    ...
    surface = surface.sort_values("joint_err").reset_index()
...
def plot_surface(surface, fit, preset):
    ...
    grid = surface.pivot(index="fraction", columns="M", values="joint_err")
    ...
    figure.tight_layout()
    path = OUTPUT_DIR / f"joint_{preset}_surface.png"
...
def plot_constraints(surface, fit, preset):
    ...
    axis.grid(alpha=0.3)
    figure.tight_layout()
    path = OUTPUT_DIR / f"joint_{preset}_constraints.png"
    figure.savefig(path, dpi=150)
    plt.close(figure)
    return path
