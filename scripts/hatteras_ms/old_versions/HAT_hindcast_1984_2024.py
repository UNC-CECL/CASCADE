...
# --- Topo/dune groin_init year ---
# TOPO_DUNE_INIT_YEAR : year label used in the *filename* (e.g. domain_7_topography_2009.npy)
# TOPO_DUNE_SUBFOLDER : subdirectory under topography/ and dunes/ where files live
#                       Change TOPO_DUNE_SUBFOLDER to switch between versions (v1, v2, etc.)
...
TOPO_DUNE_SUBFOLDER = "2009\\2009_v2"    # subfolder under topography/ and dunes/
                                          # e.g. topography\2009_v2\domain_7_topography_2009.npy
...
RUN_NAME_SUFFIX = "FinalSS"   # <- edit this to label each experiment
...
PERIOD_CONFIG = {
    1984: dict(
        ...
        storm_file           = os.path.join(
            HATTERAS_DATA_BASE, "storms", "hindcast_storms", "base_storms",
    ...
    2004: dict(
        ...
        road_setback_file    = os.path.join(
            HATTERAS_DATA_BASE, "road_offset", "raw_offset", "2004", "RoadSetback_2004.csv",
        ...
        nourishment_volume   = 100,     # m^3/m default passed to Cascade groin_init (threshold unused)
...
DUNE_OFFSET_FILE     = _cfg["dune_offset_file"]
...
print(f"Dune raw_offset file:     {os.path.basename(DUNE_OFFSET_FILE)}")
...
try:
    ...
    dune_offset_dam = dune_offset_all / 10.0  # m -> dam (single-column file, 1 value per domain)
...
def write_run_metadata(run_dir, run_name, Hs):
    ...
    lines = [
        ...
        f"dune_offset_file      = {os.path.basename(DUNE_OFFSET_FILE)}",
        "",
        "[Morphodynamics]",
