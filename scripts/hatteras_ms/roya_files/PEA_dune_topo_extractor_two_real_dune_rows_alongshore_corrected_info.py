...
VERSION = "Roya_1996_cross_ROAD_2row_alongshore_corrected"
DEM_YEAR = "2011"
...
# rows. Row 0 is the detected/used crest; row 1 is the next landward cell.
...
def extracted_dune_cell_info(
    ...
    """Describe the exact source cells that will become the saved dune rows."""
    profile = np.asarray(profile, dtype=float)
    rows = []
    for q in range(DUNE_ROWS):
        cell = int(used_crest_cell + q)
        valid = (
            0 <= cell < profile.size
        ...
        height = (
            max(elevation - berm_mhw, MIN_DUNE_H_M)
            if valid else MIN_DUNE_H_M
        )
        rows.append({
            "row": q,
            "cell": cell,
...
def pre_extraction_cross_section_qc(
    ...
    ax_map.plot(
        ...
        label="dune row 0: detected/used crest",
    )
    if DUNE_ROWS > 1:
        ax_map.plot(
            x_along,
            np.where(dune_loc_used >= 0, dune_loc_used + 1, np.nan),
            color=DUNE_ROW_COLORS[1], lw=1.3, marker=".", ms=4,
            label="dune row 1: next landward cell",
        )
            ...
            for info in dune_cells:
                ...
                if info["valid"]:
                    ...
                    row_lines.append(
                        ...
                        f"{MIN_DUNE_H_M:.2f} m minimum height"
                    )
            dune_info_text = (
    ...
    def redraw():
        ...
        status_artist.set_text(
            ...
            f"dune rows = source cells "
            f"{int(dune_loc_used[i])}–{int(dune_loc_used[i]) + DUNE_ROWS - 1}   |   "
...
def qc_figure(stem, prof_arr, start_beach, res, fig_dir: Path, road_mask=None):
    ...
    ax_map.plot(x, dl, color="#6A1B9A", lw=1.1, marker=".", ms=4,
                label="dune row 0: detected crest")
    dune_used = np.asarray(res["dune_loc_used"], dtype=float)
    for q in range(1, DUNE_ROWS):
        ax_map.plot(
            x, np.where(dune_used >= 0, dune_used + q, np.nan),
            color=DUNE_ROW_COLORS[q % len(DUNE_ROW_COLORS)],
            lw=1.1, marker=".", ms=4,
            label=f"dune row {q}: source cell crest+{q}",
        )
    ...
    for info in dune_cells:
        ...
        if info["valid"]:
            dune_row_text.append(
                f"Dune row {q}: source cell {cell}, "
                f"z={info['elevation_mhw']:.2f} m MHW, "
                f"h={info['height_above_berm']:.2f} m"
            )
        else:
            dune_row_text.append(
                f"Dune row {q}: source cell {cell}, invalid -> "
                f"h={MIN_DUNE_H_M:.2f} m"
            )
    analysis_text = (
        f"EXACT DUNE CELLS TO EXTRACT\n"
