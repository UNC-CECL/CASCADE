...
def _safe_domain_ts(b3d_obj, t):
    """
    Return a 2D interior domain for time t.
    Priority: DomainTS[t] -> InteriorDomainTS[t] -> nearest previous valid -> current InteriorDomain.
    Raises if nothing is available.
    """
    # 1) direct access
    ts = getattr(b3d_obj, "DomainTS", None)
    if ts is not None and 0 <= t < len(ts) and ts[t] is not None:
        return np.array(ts[t], copy=True)

    its = getattr(b3d_obj, "InteriorDomainTS", None)
    if its is not None and 0 <= t < len(its) and its[t] is not None:
        return np.array(its[t], copy=True)

    # 2) walk backwards for last valid snapshot
    if ts is not None:
        for k in range(min(t, len(ts)-1), -1, -1):
            if ts[k] is not None:
                return np.array(ts[k], copy=True)
    if its is not None:
        for k in range(min(t, len(its)-1), -1, -1):
            if its[k] is not None:
                return np.array(its[k], copy=True)

    # 3) fall back to the current live grid
    cur = getattr(b3d_obj, "InteriorDomain", None)
    if cur is not None:
        return np.array(cur, copy=True)

    raise RuntimeError(f"No interior domain available at or before t={t}")

def _safe_dunes_ts(b3d_obj, t):
    """
    Return a 2D dunes grid for time t, with a reasonable fallback.
    Prefers DuneDomain[t,:,:]; if out-of-range, uses last available; else zeros of interior shape.
    """
    dunes = getattr(b3d_obj, "DuneDomain", None)
    if dunes is not None and dunes.ndim == 3:
        if 0 <= t < dunes.shape[0]:
            return np.array(dunes[t, :, :], copy=True)
        # use last available slice if t is beyond what we have
        return np.array(dunes[min(dunes.shape[0]-1, max(0, t)), :, :], copy=True)

    # fallback: zeros matching interior shape
    interior = _safe_domain_ts(b3d_obj, t)
            ...
            if 0 < t <= TMAX_SIM:
                ...
                for iB3D in range(ny):
                    ...
                    if beach_management_ny[iB3D] and (t < TMAX_MGMT[iB3D] + 1):
                        ...
                        # ) * 10
                        Domain = _safe_domain_ts(barrier3d[iB3D], t) * 10
                        Dunes = (_safe_dunes_ts(barrier3d[iB3D], t) + barrier3d[iB3D].BermEl) * 10
...
os.chdir('/Users/rsahrae/PycharmProjects/CASCADE_Coupling/CASCADE/data/Inlet_dataset/Run_Output/')
run_name = "Connected_25_Domain_Inlet_Natural_Run"
name_prefix = run_name
nt_run = 130 # Number of years to plot
number_barrier3d_models = 24 # Number of B3D domains to plot
Model_Grids_Of_Interest = range(0,24) # Specific B3D domains to plot
...
directory = "/Users/rsahrae/PycharmProjects/CASCADE_Coupling/CASCADE/data/Inlet_dataset/Run_Output/"
# TMax_MGMT = Needed 0
