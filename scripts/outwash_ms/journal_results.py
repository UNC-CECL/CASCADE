...
for rname in rname_array:
    ...
    for storm_num in range(1, 101):
        ...
        if b3d_obj.barrier3d[0].drown_break == 0:
            ...
            new_ave_interior_height = np.average(domain_TS)  # all in m MHW, average value of the domain for this storm sequence
            avg_int_height_array_b3d[storm_num-1] = new_ave_interior_height  # array of the average elevations for all storm sequences

        # width - only for barriers that do not drown
        if b3d_obj.barrier3d[0].drown_break == 0:
            int_width_TS = np.array(b3d_obj.barrier3d[0].InteriorWidth_AvgTS) * 10  # meters, already averaged values
            last_int_width = int_width_TS[-1]  # the width at the final time step
            # avg_width_last_TS_b3d.append(last_int_width)
            avg_int_width_array_b3d[storm_num-1] = int_width_TS[-1]  # array of the average widths for all storm sequences

        # dune crest
        sub_domain = b3d_obj.barrier3d[0]._DuneDomain[0:tmax_b3d, :, :]  # dune domains for current storm sequence, all model years
        dune_crest = sub_domain.max(axis=2)  # n rows are simulation years, n cols are alongshore length (dune crests)
        dune_crest_avg = np.average(dune_crest) * 10  # average of all years, converted to meters
        dune_crest_array_b3d[storm_num-1] = dune_crest_avg  # array of the average dune crests for all storm sequences
        ...
        Qow_avg = np.mean(QowTS)  # m^3/m/yr average overwash flux for this storm sequence
        # overwash_array_b3d[storm_num-1] = np.array(QowTS)
        avg_overwash_array_b3d[storm_num-1] = Qow_avg  # array of the average overwash fluxes
        ...
        # shoreline position - only for barriers that do not drown
        m_xsTS = np.subtract(outwash100_obj.barrier3d[0].x_s_TS, outwash100_obj.barrier3d[0].x_s_TS[0])
        m_xsTS = np.multiply(m_xsTS, 10)
        ...
        if outwash100_obj.barrier3d[0].drown_break == 0:
            ...
            while check == 1:
                if all(x <= 0 for x in domain_TS[-1, :]):
                    ...
                    check = 0
            domain_TS[domain_TS < 0] = 0  # set all values less than 0 to 0
            new_ave_interior_height = np.average(domain_TS)  # m MHW, avg value of the domain for this storm sequence
            avg_int_height_array_100[storm_num - 1] = new_ave_interior_height  # array of the avg elev for all storm sequences

        # width - only for barriers that do not drown
        if outwash100_obj.barrier3d[0].drown_break == 0:
            int_width_TS = np.array(outwash100_obj.barrier3d[0].InteriorWidth_AvgTS) * 10  # meters, already averaged
            last_int_width = int_width_TS[-1]  # the width at the final time step
            avg_int_width_array_100[storm_num - 1] = int_width_TS[-1]  # array of the avg widths for all storm sequences

        # dune crest
        sub_domain = outwash100_obj.barrier3d[0]._DuneDomain[0:tmax_100, :,
                     :]  # dune domains for current storm sequence, all model years
        dune_crest = sub_domain.max(
            axis=2)  # n rows are simulation years, n cols are alongshore length (dune crests)
        dune_crest_avg = np.average(dune_crest) * 10  # average of all years, converted to meters
        dune_crest_array_100[
        ...
        Qow_avg = np.mean(QowTS)  # m^3/m/yr average overwash flux for this storm sequence
        avg_overwash_array_100[storm_num-1] = Qow_avg  # array of the average overwash fluxes

        # outwash
        QoutTS = outwash100_obj.outwash[0]._outwash_flux_TS  # m^3/m, all model years for this storm sequence
        QoutTS = QoutTS[QoutTS!=0]  # remove all nonzero values (they should have been initialized as nans)
        Qout_avg = np.mean(QoutTS)  # m^3/m/yr average outwash flux for this storm sequence
        avg_outwash_array_100[storm_num-1] = Qout_avg  # array of the average outwash fluxes
        ...
        # shoreline position - only for barriers that do not drown
        m_xsTS = np.subtract(outwash50_obj.barrier3d[0].x_s_TS, outwash50_obj.barrier3d[0].x_s_TS[0])
        m_xsTS = np.multiply(m_xsTS, 10)
        ...
        if outwash50_obj.barrier3d[0].drown_break == 0:
            ...
            domain_TS[domain_TS<0] = 0  # set all values less than 0 to 0
            new_ave_interior_height = np.average(domain_TS)  # all in m MHW, average value of the domain for this storm sequence
            avg_int_height_array_50[storm_num-1] = new_ave_interior_height  # array of the average elevations for all storm sequences

        # width - only for barriers that do not drown
        if outwash50_obj.barrier3d[0].drown_break == 0:
            int_width_TS = np.array(outwash50_obj.barrier3d[0].InteriorWidth_AvgTS) * 10  # meters, already averaged values
            last_int_width = int_width_TS[-1]  # the width at the final time step
            avg_int_width_array_50[storm_num-1] = int_width_TS[-1]  # array of the average widths for all storm sequences

        # dune crest
        sub_domain = outwash50_obj.barrier3d[0]._DuneDomain[0:tmax_50, :, :]  # dune domains for current storm sequence, all model years
        dune_crest = sub_domain.max(axis=2)  # n rows are simulation years, n cols are alongshore length (dune crests)
        dune_crest_avg = np.average(dune_crest) * 10  # average of all years, converted to meters
        ...
        Qow_avg = np.mean(QowTS)  # m^3/m/yr average overwash flux for this storm sequence
        avg_overwash_array_50[storm_num-1] = Qow_avg  # array of the average overwash fluxes

        # outwash
        QoutTS = outwash50_obj.outwash[0]._outwash_flux_TS  # m^3/m, all model years for this storm sequence
        QoutTS = QoutTS[QoutTS != 0]
        Qout_avg = np.mean(QoutTS)  # m^3/m/yr average outwash flux for this storm sequence
        avg_outwash_array_50[storm_num-1] = Qout_avg  # array of the average outwash fluxes
        ...
        # shoreline position - only for barriers that do not drown
        m_xsTS = np.subtract(outwash0_obj.barrier3d[0].x_s_TS, outwash0_obj.barrier3d[0].x_s_TS[0])
        m_xsTS = np.multiply(m_xsTS, 10)
        ...
        if outwash0_obj.barrier3d[0].drown_break == 0:
            ...
            domain_TS[domain_TS < 0] = 0  # set all values less than 0 to 0
            new_ave_interior_height = np.average(domain_TS)  # all in m MHW, average value of the domain for this storm sequence
            avg_int_height_array_0[storm_num - 1] = new_ave_interior_height  # array of the average elevations for all storm sequences

        # width - only for barriers that do not drown
        if outwash0_obj.barrier3d[0].drown_break == 0:
            int_width_TS = np.array(
                outwash0_obj.barrier3d[0].InteriorWidth_AvgTS) * 10  # meters, already averaged values
            last_int_width = int_width_TS[-1]  # the width at the final time step
            avg_int_width_array_0[storm_num - 1] = int_width_TS[-1]  # array of the average widths for all storm sequences

        # dune crest
        sub_domain = outwash0_obj.barrier3d[0]._DuneDomain[0:tmax_0, :,
                     :]  # dune domains for current storm sequence, all model years
        dune_crest = sub_domain.max(axis=2)  # n rows are simulation years, n cols are alongshore length (dune crests)
        dune_crest_avg = np.average(dune_crest) * 10  # average of all years, converted to meters
        ...
        Qow_avg = np.mean(QowTS)  # m^3/m/yr average overwash flux for this storm sequence
        avg_overwash_array_0[storm_num - 1] = Qow_avg  # array of the average overwash fluxes

        # outwash
        QoutTS = outwash0_obj.outwash[0]._outwash_flux_TS  # m^3/m, all model years for this storm sequence
        QoutTS = QoutTS[QoutTS != 0]
        Qout_avg = np.mean(QoutTS)  # m^3/m/yr average outwash flux for this storm sequence
        avg_outwash_array_0[storm_num - 1] = Qout_avg  # array of the average outwash fluxes
    ...
    if geomoetry_stats:
        # elevation
        print("baseline avg interior elevation excluding drown years: {0}".format(avg_height_last_TS_b3d))
        print("100% outwash avg interior elevation excluding drown years: {0}".format(avg_height_last_TS_100))
        print("50% outwash avg interior elevation excluding drown years: {0}".format(avg_height_last_TS_50))
        print("0% outwash avg interior elevation excluding drown years: {0}".format(avg_height_last_TS_0))
        # width
        print("baseline avg interior width excluding drown years: {0}".format(avg_width_last_TS_b3d))
    ...
    if flux_stats:
        ...
        print("average overwash per event 100%: {0}".format(avg_overwash_array_100))
        print("average overwash per event 50%: {0}".format(avg_overwash_array_50))
        print("average overwash per event 0%: {0}".format(avg_overwash_array_0))
