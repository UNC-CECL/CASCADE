...
# CASCADE x_s_TS unit — determines how LRR is converted to m/yr before residual computation
# 'dam' : x_s_TS in decameters → multiply by 10 to get m/yr  (most likely for CASCADE/Barrier3D)
# 'm'   : x_s_TS already in meters → no conversion needed
# Check the "Sample LRR values" printout: if they're ~0.01-0.5 and CoastSat is ~-5 to +4 m/yr,
# the units are almost certainly 'dam'. If they're similar in scale to CoastSat, use 'm'.
...
# Domains included in RMSE evaluation — only where corrections are physically applied.
# Avon zone (25-30) + Wimble Shoals shadow + Rodanthe bell (38-89)
# Using the full island dilutes the signal with domains the corrections can't affect.
WIMBLE_EVAL_DOMAINS = list(range(25, 31)) + list(range(38, 90))

# No exclusion zones within the correction domains
EXCLUSION_ZONES = set()
...
# Narrow these ranges around the best-fit values after a first pass.
# =============================================================================
...
def compute_residual(coastsat_lrr, cascade_lrr, domains,
    ...
    Unit handling:
      - CoastSat LRR is always in m/yr (from your CSV)
      - CASCADE x_s_TS LRR is in dam/yr if CASCADE_X_S_UNITS='dam' (most common)
        or m/yr if CASCADE_X_S_UNITS='m'
      - Both are converted to m/yr before subtraction, then result is
        divided by 10 to express the residual in dam/yr for comparison
        with the parametric correction (also in dam/yr)

    Positive residual → CoastSat more accretional than CASCADE → need source term
    Negative residual → CoastSat more erosional than CASCADE  → need sink term
    """
    ...
    for d in domains:
        ...
        if not (np.isnan(cs) or np.isnan(cas)):
            cas_m = cas * m_per_unit        # convert CASCADE LRR to m/yr
            residual[d] = (cs - cas_m) / 10.0   # residual in dam/yr
    return residual
...
if __name__ == "__main__":
    ...
    # ── Unit check — residuals should be comparable in scale to sweep ranges ──
    print(f"\nUnit check (CASCADE_X_S_UNITS = '{CASCADE_X_S_UNITS}'):")
    for label, dom_range in [
        ("Wimble Shoals zone (38-80)", list(range(38, 81))),
        ("Avon zone (25-30)",          list(range(25, 31))),
        ("Rodanthe zone (74-84)",      list(range(74, 85))),
    ]:
    ...
    print(f"  Sweep range: WS {WS_PEAK_VALUES[0]}–{WS_PEAK_VALUES[-1]} | "
          f"Rod {ROD_AMPLITUDES[-1]}–{ROD_AMPLITUDES[0]} | "
          f"Avon {AVON_NORTH_VALUES[0]}–{AVON_NORTH_VALUES[-1]} dam/yr")
    print(f"  → If residuals >> sweep range, set CASCADE_X_S_UNITS = 'm'")
    print(f"  → If residuals << sweep range, narrow sweep ranges further")
