...
#
# Usage:
#   1. Point STORM_SUMMARY_CSV to your storm_summary_<PERIOD>.csv comparison from Storms_v2.py
...
STORM_SUMMARY_CSV = Path(r"C:\Users\hanna\PycharmProjects\CASCADE\data\hatteras_init\storms\hindcast_storms\roya_storms_v2\1984_2004\storm_summary_storms_1984_2004.csv")
...
OUTPUT_PATH = Path(
    r"/scripts/input_prep/storm_creation_final/from_Hannah/storm_creation/storm_figures/output_roya/storm_record_1984_2004_fixed.png")
SAVE_FIGURE = True
SHOW_FIGURE = True
...
def load_data(csv_path, begin_year, end_year):
    """
    Load storm_summary_<PERIOD>.csv produced by Storms_v2.py and normalise column
    names to the internal convention used throughout this script.

    Storms_v2.py columns    → internal name used here
    --------------------   --------------------------
    calendar_year        → Calendar_Year
    StartTime            → Storm_Start
    EndTime              → Storm_End
    StartTime            → Peak_TWL_Time  (best available proxy; no separate peak column)
    Rhigh * 10           → Rhigh_m        (Storms_v2.py stores dam; convert back to metres)
    Peak_Hs_m            → Peak_Hs_m      (column absent — filled with 1.0 as neutral size)
    """
    df = pd.read_csv(csv_path)

    # Drop any stray index columns written by pandas
    ...
    for col in ['Storm_Start', 'Storm_End']:
        df[col] = pd.to_datetime(df[col])

    # Peak_TWL_Time: use Storm_Start as the best available proxy
    df['Peak_TWL_Time'] = df['Storm_Start']

    # Rhigh_m: Storms_v2.py saves Rhigh in decameters — multiply by 10 for metres
    df['Rhigh_m'] = df['Rhigh'] * 10

    # Peak_Hs_m: not written by Storms_v2.py — use neutral placeholder for marker sizing
    if 'Peak_Hs_m' not in df.columns:
        df['Peak_Hs_m'] = 1.0
...
def main():
    df             = load_data(STORM_SUMMARY_CSV, BEGIN_YEAR, END_YEAR)
