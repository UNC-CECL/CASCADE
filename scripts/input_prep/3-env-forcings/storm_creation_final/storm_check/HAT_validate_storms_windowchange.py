...
READABLE_CSV = Path(r"/data/hatteras_init/storms/hindcast_storms/base_storms/storms_1984_2004_base_readable.csv")
...
def match_storms(hannah_df: pd.DataFrame, historical: list,
    ...
    for storm in historical:
        ...
        if len(candidates) == 0:
            ...
            # Timing raw_offset: hours outside the active storm window.
            # 0 = peak TWL falls within the HURDAT2 active period (ideal).
            # Negative = peak TWL before storm start (Duck led Hatteras).
            # Positive = peak TWL after storm end (Duck lagged Hatteras).
            # Using nearest edge avoids penalising long-track storms like
            # Isabel (active 2 weeks at sea before hitting Hatteras) or
            # Kyle (erratic, long-lived) where the midpoint is misleading.
            peak = best_row['Peak_TWL_Time']
            if peak < storm['start_ts']:
                offset_h = (peak - storm['start_ts']).total_seconds() / 3600
            elif peak > storm['end_ts']:
                offset_h = (peak - storm['end_ts']).total_seconds() / 3600
            else:
                offset_h = 0.0  # peak falls within the active storm window
