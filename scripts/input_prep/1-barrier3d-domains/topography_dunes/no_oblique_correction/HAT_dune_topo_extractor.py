...
# --- MODE ---------------------------------------------------------------
MODE = "run"      # "pick" | "run" | "pick_and_run"
PICK_DOMAINS = None        # None = all found, or e.g. list(range(1, 31))
...
VERSION = "v1"             # bump this per settings variant, like Lexi's sheet
DEM_YEAR = "2009"
...
# --- INTERIOR -----------------------------------------------------------
USE_CONST_INTERIOR = True  # interior starts one cell landward of the MOST landward
...
def load_profiles(in_path: Path) -> dict:
    ...
    Load one domain. Returns a dict:
        ...
        start_beach : (n_along,) first index in z where z > BEACH_START_THR_M,
                      -1 if none
        c0          : cross-shore raw_offset of z within raw, i.e. z[:, k] == raw
                      column k + c0. Lets the figures overlay picks on the raw.
    """
    ...
    # trim in ocean-first coords (identical result to v3's trim-then-flip, but the
    # raw_offset stays addressable so the raw and processed arrays can be co-plotted)
...
def comparison_figure(stem, dom, res, fig_dir: Path):
    """
    Side-by-side of the raw GIS domain and the arrays CASCADE actually receives.

    Left  : raw DEM as loaded from GIS (m NAVD88, untrimmed), ocean at bottom,
            with MHW, the trim edge, the dune search window, the picked crests
            and the interior start line overlaid.
    Right : the saved dune + interior arrays, converted back to m for display,
            stacked the way Barrier3D sees them (dune row at the ocean edge,
            interior landward of it).
    """
    raw = dom["raw"]
    c0 = dom["c0"]
    ...
    # overlays live in trimmed coords -> shift by c0 to land on the raw axis
    dl_raw = np.where(res["dune_loc"] >= 0, res["dune_loc"] + c0, np.nan)
    sb_raw = np.where(dom["start_beach"] >= 0, dom["start_beach"] + c0, np.nan)

    topo_m = res["topo_dm"] * 10.0      # back to m MHW for display
    ...
    # ---------------- left: raw GIS ----------------
    # Shared display window with the interior panel so the two are comparable by
    # eye: same elevation span, just labelled in NAVD88 vs MHW. Scaling the raw
    # to its own 1-99 percentile lets the -10 m shoreface swamp the ramp and the
    # island reads as one flat colour.
    land = raw[raw > MHW_M]
    hi_navd = float(np.percentile(land, 99)) if land.size else MHW_M + 4.0
    vmin_r = MHW_M - 1.0
    vmax_r = max(hi_navd, MHW_M + 2.0)
    # raw is (n_along, n_cross) -> transpose so rows map to the cross-shore y axis
    ...
    ax_raw.axhspan(i0 + c0, i1 + c0, color="#FF8C00", alpha=0.22, zorder=0)
    ax_raw.plot(np.arange(n_along), sb_raw, color="k", lw=1.0, label="beach start")
    ax_raw.plot(np.arange(n_along), dl_raw, color="#B71C1C", lw=0, marker=".",
                ms=4, label="picked dune crest")
    if res["start_island"] is not None:
        ax_raw.axhline(res["start_island"] + c0, color="w", lw=1.4, ls="--",
                       label="interior start")
    ...
    fig.suptitle(
        ...
        f"interior start row {res['start_island']}",
        fontsize=12,
    )
    ...
    if MODE in ("pick", "pick_and_run"):
        for name in names:
            ...
            if not REPICK_EXISTING and stem in windows:
                w = windows[stem]
                print(f"[skip pick] {stem}: window [{w['i0']}, {w['i1']}] already saved")
                continue
            try:
