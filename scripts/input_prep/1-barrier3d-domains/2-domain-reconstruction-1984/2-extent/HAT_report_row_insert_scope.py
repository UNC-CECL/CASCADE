...
def fig_grid(rows, topo_dir):
    ...
    for k, (ax, g) in enumerate(zip(axes, groups)):
        ...
        for d in g:
            ...
            if ANCHOR == "road" and np.isfinite(sb_new_by.get(d, np.nan)):
                ...
                ax.add_patch(Rectangle((d - 0.32, y_old), 0.64, 2.0, facecolor="none",
                                       edgecolor="0.25", linewidth=0.7, zorder=9))
            elif np.isfinite(sb):
    ...
    handles += [Patch(facecolor="white", edgecolor="0.4", linewidth=0.4, label="beyond the domain"),
                ...
                (Patch(facecolor="none", edgecolor=C_ADD, linewidth=0.9,
                       label="rows inserted landward of NC-12, filled by copying the adjacent landward rows")
                 if FILL == "copy" and ANCHOR == "road" else
                 Patch(facecolor=C_ADD_FILL, edgecolor="none",
                       label=("rows inserted landward of NC-12 (unfilled)"
                              if ANCHOR == "road" else "rows inserted at the seaward edge (unfilled)"))),
                Patch(facecolor="none", edgecolor=C_REM, hatch="//////",
                      label=("rows removed seaward of NC-12" if ANCHOR == "road"
                             else "rows removed at the seaward edge")),
                Patch(facecolor=C_ROAD, edgecolor="none",
                      label=("NC-12 road rows, measured position" if ANCHOR == "dune"
                             else "NC-12 road rows at the 1984 setback (model input)")),
                Line2D([0], [0], color=off.HATTERAS_ANNOTATIONS.color_town_span, lw=4.0, label="community")]
    if ANCHOR == "road":
        handles.append(Patch(facecolor="none", edgecolor="0.25", linewidth=0.7,
    ...
    if FILL == "copy" and ANCHOR == "road":
        p = insert_figures_dir(PRODUCT, "4-fill", "island") / "HAT_fill_copy_grid_island.png"
    else:
...
def write_report(rows, topo_name, path, fig_path):
    ...
    w(f"plan view      figures/3-placement/seaward/HAT_footprint_1984_plan.png  (HAT_footprint_1984.py)")
    w("")
    ...
    w("  2. THE REMOVED ROWS ARE SURVEYED. Where the island prograded, the seaward")
    w("     placement drops rows 0..|N|-1 (1996 beach and foredune that were ocean in")
    w("     1984); the behind-road placement drops the |N| rows directly seaward of")
    w("     today's roadway rows. Either deletes measured cells on the strength of")
    w("     the line pair.")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--fill", choices=("blank", "copy"), default="blank",
                    help="copy: draw the block FILLED by the copy rule (the next N rows), "
                         ...
                         "4-fill/HAT_fill_copy_grid_island.png")
    a = ap.parse_args()
