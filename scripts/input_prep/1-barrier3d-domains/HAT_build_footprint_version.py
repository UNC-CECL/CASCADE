...
WHAT IS WRITTEN (dune-topo/<dst>/)
    ...
    RoadSetback_1984_dunestart.csv         the 1984 setbacks: setback_new_m from
                                           ...
                                           inland, onto the copied block, and by
                                           |N| rows seaward where rows were
                                           removed (Hannah, 2026-09-08). Domains
...
VERIFIED AFTER WRITING
    ...
    v2, and its block equals the rows that follow it.
...
from hat_topo_version import array_name, dune_topo_root, topo_dirs  # noqa: E402

PRODUCT = "1984-start"
SRC_VERSION = "v2"
SCOPE = INIT / "1-barrier3d-domains" / PRODUCT / "row-insert-scope"
FOOTPRINT_CSV = SCOPE / "footprint_1984_by_domain.csv"
...
def main() -> None:
    ...
    (dst / "RUN_MANIFEST.txt").write_text(
        ...
        f"builder    : scripts/input_prep/1-barrier3d-domains/HAT_build_footprint_version.py\n\n"
        ...
        "  remove       rows insert..insert+|N|-1 deleted\n"
    ...
    (dst / "README.md").write_text(
        ...
        f"`row-insert-scope/footprint_1984_by_domain.csv`. **Nothing is re-measured here**: the "
        f"footprint (`HAT_footprint_1984.py`) and the fill (`HAT_fill_copy_scope.py`) carry the "
        f"argument; this folder applies them.\n\n"
        f"| | |\n|---|---|\n"
        f"| what | `{src_name}` + the symmetric 1984 footprint placed behind NC-12, filled by copying the N rows that follow the insert point |\n"
        ...
        f"rows_before + N rows, identical rows before the insert point, and a block equal to the rows "
        f"that follow it.\n\n"
    ...
    (dst / "RoadSetback_README.md").write_text(
        ...
        f"`row-insert-scope/footprint_1984_by_domain.csv` (`setback_new_m` = (road - row 0) + the paired "
