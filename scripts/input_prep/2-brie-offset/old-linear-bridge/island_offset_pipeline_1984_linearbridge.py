...
def calculate_relative_offset(file_path, year, col_map, grids):
    """
    Perform the absolute-to-relative raw_offset calculation for a single year.
...
def pad_for_cascade(df, padding_zeros, target_length):
    """
    Pad a DataFrame of raw_offset values for CASCADE using a smooth bridge
    ...
    ----------
    df : pd.DataFrame
        Single raw_offset column for real domains, ordered from Domain 1 to Domain N.
    padding_zeros : int
...
def main():
    ...
    # CASCADE-format: raw_offset column only (no Domain_ID)
