...
def main(argv=None):
    ...
    if have_raw:
        ...
        out[f"abs_{args.a}_m"] = ra.reindex(out.index)
        out[f"abs_{args.b}_m"] = rb.reindex(out.index)
    ...
    moved = d[d.abs() >= 0.5]
    print(f"{args.year} {args.a} -> {args.b}")
    if have_raw:
        ...
        print(f"  baseline shift ({args.b} - {args.a} of the per-year minimum): "
    ...
    ax = axes[0]
    ax.plot(x, out[f"model_{args.a}_m"], color=C_1984, lw=1.2, label=args.a)
    ax.plot(x, out[f"model_{args.b}_m"], color=C_1997, lw=1.2, label=args.b)
    ...
    cap = (f"Two builds of the {args.year} island offset. (a) The unpadded offset each "
           f"build hands the model, {args.a} in red and {args.b} in blue, each zeroed on "
           f"its own most seaward domain. ")
    if have_raw:
        cap += (f"(b) The change in the dune line itself, {args.b} minus {args.a}, measured "
                ...
                f"largest, {d.abs().max():+.1f} m, at GIS {d.abs().idxmax()}). Both raw "
        ...
        cap += f"(b) {args.b} minus {args.a} in that model frame."
    caption(fig, cap)
