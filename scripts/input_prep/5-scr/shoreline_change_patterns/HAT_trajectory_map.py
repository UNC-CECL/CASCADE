...
def plot_trajectory_map(domain_gdf, outline_gdf, metrics, out_path):
    ...
    for _, row in domain_gdf.iterrows():
        ...
        if dom in metrics.index and metrics.loc[dom, "high_var"]:
            gpd.GeoSeries([row.geometry], crs=PLOT_CRS).plot(
                ax=ax, color="none", edgecolor="#FFD700",
    ...
    patches.append(mpatches.Patch(facecolor="none", edgecolor="#FFD700",
...
def plot_delta_lrr_map(domain_gdf, outline_gdf, metrics, out_path):
    ...
    # Figure: single map + colorbar below + small notes panel
    fig = plt.figure(figsize=(9, 18))
    gs  = plt.GridSpec(2, 1, height_ratios=[15, 0.9],
                       hspace=0.03,
    ...
    for _, row in domain_gdf.iterrows():
        dom  = row["domain"]
        dlrr = delta.get(dom, np.nan)
        if np.isnan(dlrr):
            color, alpha = "#666666", 0.4
        else:
            color = lrr_cmap(norm(dlrr))
            # Opacity encodes magnitude of change — bigger shift = more opaque
            mag   = min(abs(dlrr) / vabs, 1.0)
            alpha = 0.50 + 0.45 * mag
    ...
    ax.set_axis_off()
    ax.set_title(
        "Hatteras Island  |  Change in shoreline trend between periods\n"
        "ΔLRR = Period 2 (2004–2024) − Period 1 (1984–2004)  ·  CoastSat",
        fontsize=10, fontweight="bold", pad=6)
    ...
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax_cb, orientation="horizontal",
                        fraction=0.8, shrink=0.60, aspect=35, pad=0.0)
    cbar.set_label(
        "ΔLRR  (m/yr)  —  "
        "blue = accreting more (or eroding less) in P2  ·  "
        "red = eroding more (or accreting less) in P2",
        fontsize=8)
    cbar.ax.tick_params(labelsize=7)
...
def main():
    ...
    plot_delta_lrr_map(
        domain_gdf, outline_gdf, metrics,
        os.path.join(OUTPUT_DIR, "fig_map_delta_lrr.png"))
