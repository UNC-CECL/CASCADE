...
CLASS_COLORS = {
    ...
    "Decelerating Erosion":   "#f4a582",   # lightest salmon — improving
    "Persistently Stable":    "#4dac26",   # green
    "Decelerating Accretion": "#92c5de",   # lightest blue  — fading
    "Accelerating Accretion": "#4393c3",   # mid blue       — getting better
    "Persistent Accretion":   "#2166ac",   # darkest blue   — ongoing, high confidence
    "Switching: Acc→Ero":     "#762a83",   # dark purple    — full reversal
...
def classify(lrr1, lrr2, thr=THRESHOLD):
    ...
    e2 = lrr2 < -thr;  a2 = lrr2 > thr;  s2 = not e2 and not a2
    # Persistent
    ...
    # Decelerating (was active, now stable)
    elif e1 and s2:  return "Decelerating Erosion"
    elif a1 and s2:  return "Decelerating Accretion"
    # Accelerating (was stable, now active)
    elif s1 and e2:  return "Accelerating Erosion"
    elif s1 and a2:  return "Accelerating Accretion"
...
def plot_classification_bar(metrics, out_path):
    ...
    axes[2].legend(handles=patches, fontsize=6, loc="upper right",
                   ncol=5, framealpha=0.9, title="Trajectory class", title_fontsize=6)
...
def plot_lrr_scatter(metrics, out_path):
    ...
    for ax, lrr_col_x, lrr_col_y, label_x, label_y, title_suffix in [
        ...
        # Quadrant labels — now includes acceleration/deceleration zones
        for tx, ty, label in [
            (lims[0]*0.75, lims[1]*0.75, "Persistent\nErosion"),
            (lims[1]*0.75, lims[1]*0.75, "Switching\nEro→Acc"),
            (lims[0]*0.75, lims[0]*0.75, "Switching\nAcc→Ero"),
            (lims[1]*0.75, lims[0]*0.75, "Persistent\nAccretion"),
            (lims[0]*0.35, lims[1]*0.35, "Decelerating\nErosion"),
            (lims[1]*0.35, lims[0]*0.35, "Decelerating\nAccretion"),
            (lims[0]*0.35, lims[0]*0.35, "Accelerating\nAccretion"),
            (lims[1]*0.35, lims[1]*0.35, "Accelerating\nErosion"),
        ]:
    ...
    fig.legend(handles=patches, fontsize=7, loc="lower center",
               ncol=5, framealpha=0.9, title="Trajectory class",
               title_fontsize=7, bbox_to_anchor=(0.5, -0.04))
...
def plot_hovmoller(metrics, out_path):
    ...
    for dom in domains:
        ...
        if metrics.loc[dom, "high_var"]:
            ax_cl.bar(dom, 0.25, width=0.95, bottom=0.75,
                      color="#FFD700", zorder=3, alpha=0.9)
    ...
    patches = [mpatches.Patch(color=v, label=k)
               for k, v in CLASS_COLORS.items() if k != "Insufficient Data"]
    patches.append(mpatches.Patch(color="#FFD700", alpha=0.9, label="High variability flag"))
