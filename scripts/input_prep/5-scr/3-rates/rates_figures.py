...
def _draw(frame, x, y, half, y_label, tick, fills, std, target=None):
    ...
    if target is not None:
        ax.plot(target.index.to_numpy(float), target.to_numpy(float),
                color=TARGET_C, lw=TARGET_LW, solid_capstyle="round", zorder=6)
...
def lrr_figures():
    ...
    for (s, e), dom in frames.items():
        ...
        fig, ax, n_out = _draw(_frame(dom, "mean_lrr", "std_lrr"), x,
                               ...
                               target=target)
        _legend(fig, "Shoreline change rate (LRR)", std=True, target=True)
        caption(fig, (
    ...
    for product, root, half, ylab, tick, std, what, stem in products:
        for chain in CHAINS:
            ...
            for i, (ax, (s, e)) in enumerate(zip(axes, chain)):
                frame, x, y, fills, (v0, v1), target = _load(product, s, e)
                cw.draw_panel(ax, frame, half, label=(i == 0), std=std)
                n_out += _dots(ax, x, y, half)
                if target is not None:
                    ax.plot(target.index.to_numpy(float), target.to_numpy(float),
                            color=TARGET_C, lw=TARGET_LW, solid_capstyle="round",
                            zorder=6)
            ...
            _legend(fig, what, std=std, target=(product == "lrr"))
            if product == "lrr":
