...
Compares domain-level Linear Regression Rates (LRR) from:
  ...
  DSAS     : LRR aggregated from DSAS transect-level comparison across
...
COASTSAT_CSV_1978_1997 = None #r"C:\Users\hanna\PycharmProjects\CASCADE\scripts\input_prep\CoastSat\1978_1997\domain_lrr_summary.csv"
...
# --- Output ---
OUTPUT_DIR = r"/scripts/input_prep/CoastSat/dsas_coastsat_specific_dates"
...
def load_dsas(path, domain_col, lrr_col, std_col, period_label):
    if path is None or not os.path.exists(path):
        print(f"DSAS ({period_label}): SKIPPED — file not found: {path}")
        return None
    df = pd.read_csv(path)
...
def plot_line_comparison(merged, period_label, out_path):
    ...
    # Use the full data range so CoastSat domains beyond DSAS aren't clipped
    x_min = min(DOMAIN_MIN, merged["domain"].min()) - 1
    x_max = max(DOMAIN_MAX, merged["domain"].max()) + 1
    ax.set_xlim(x_min, x_max)
    ax.set_xticks(range(int(x_min) + 1, int(x_max), 10))
    ax.set_xticks(range(int(x_min) + 1, int(x_max), 5), minor=True)
...
def plot_scatter(merged, period_label, out_path):
    ...
    ax.scatter(merged["dsas_lrr"], merged["cs_lrr"],
...
def plot_both_periods_overview(merged_1978, merged_1997, out_path):
    ...
    # Collect domain range across whichever periods have data
    all_domains = pd.concat([m["domain"] for m in [merged_1978, merged_1997] if m is not None])
    x_min = min(DOMAIN_MIN, all_domains.min()) - 1
    x_max = max(DOMAIN_MAX, all_domains.max()) + 1
    axes[-1].set_xlim(x_min, x_max)
    axes[-1].set_xticks(range(int(x_min) + 1, int(x_max), 10))
    axes[-1].set_xticks(range(int(x_min) + 1, int(x_max), 5), minor=True)
...
def main():
    ...
    # ---- Merge ----
    merged_1978 = merge_datasets(dsas_1978, cs_1978) if (dsas_1978 is not None and cs_1978 is not None) else None
    merged_1997 = merge_datasets(dsas_1997, cs_1997) if (dsas_1997 is not None and cs_1997 is not None) else None
