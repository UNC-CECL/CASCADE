...
STUDY_AREA_FILTER_PATH = r"/scripts/input_prep/5-scr/shoreline_inventory/cascade_area.geojson"
...
WET_DRY_PATH     = r"/scripts/input_prep/5-scr/shoreline_inventory/wet_dry_groin.geojson"
WET_DRY_DATE_COL = "date"

# --- Source 2: NC Coastal Management historical shorelines ---
NC_STATE_PATH     = r"/scripts/input_prep/5-scr/shoreline_inventory/nc_shorelines.geojson"
...
COASTSAT_ROOT_DIR = r"/scripts/input_prep/5-scr/CoastSat\coastsat_timeseries"
...
# --- Output ---
OUTPUT_DIR = r"/scripts/input_prep/5-scr/shoreline_inventory/shoreline_position_output"
...
ZOOM_REGIONS = [
    ("Cape Point / Buxton",       3_895_500, 3_912_000),
    ("Avon area",                  3_910_000, 3_930_000),
    ("Wimble Shoals coast",       3_928_000, 3_945_000),
    ("Tri-Village / Pea Island",  3_943_000, 3_963_000),
]
...
ZOOM_REGIONS = [
    ("Cape Point + Buxton",   3_895_500, 3_910_000),
    ("Buxton–Avon corridor",  3_908_000, 3_923_000),
    ("Wimble Shoals coast",   3_921_000, 3_940_000),
    ("Rodanthe + Pea Island", 3_938_000, 3_962_500),
]
...
def plot_shoreline_map_zoomed_regions(wet_dry_gdf: gpd.GeoDataFrame,
                                       nc_state_gdf: gpd.GeoDataFrame,
                                       coastsat_recon: pd.DataFrame,
                                       coastsat_alongshore: pd.DataFrame,
                                       colormap: str,
    ...
    # Rank-based coloring across all sources (shared across regions)
    all_years = []
    if len(wet_dry_gdf) > 0:
        all_years.extend(wet_dry_gdf["_year"].tolist())
    if len(nc_state_gdf) > 0:
    ...
    # Reconstruct alongshore mapping for CoastSat (needed to sort points)
    if len(coastsat_recon) > 0 and len(coastsat_alongshore) > 0:
        alongshore_map = dict(zip(coastsat_alongshore["transect_id"],
                                   coastsat_alongshore["alongshore_m"]))
        coastsat_prepped = coastsat_recon.copy()
        coastsat_prepped["alongshore_m"] = (
            coastsat_prepped["transect_id"].map(alongshore_map))
        coastsat_prepped = coastsat_prepped.dropna(subset=["alongshore_m"])
    else:
        coastsat_prepped = pd.DataFrame()
    ...
    fig = plt.figure(figsize=(4.5 * n_regions + 1.5, 15))
    gs = GridSpec(1, n_regions + 1,
                  width_ratios=[1] * n_regions + [0.08],
                  wspace=0.10,
                  top=0.93, bottom=0.06, left=0.05, right=0.97)
    panel_axes = [fig.add_subplot(gs[0, i]) for i in range(n_regions)]
    cbar_ax = fig.add_subplot(gs[0, n_regions])

    def format_km(x, _pos):
        return f"{x/1000:.1f}"

    # ─── For each zoom region ─────────────────────────────────────────────
    for i, (region_name, y_lo, y_hi) in enumerate(ZOOM_REGIONS):
        ax = panel_axes[i]
        ...
        # Wet-dry: use geometry bounds for features whose y overlaps region
        if len(wet_dry_gdf) > 0:
            for _, row in wet_dry_gdf.iterrows():
                gmin_x, gmin_y, gmax_x, gmax_y = row.geometry.bounds
                if gmax_y >= y_lo and gmin_y <= y_hi:
                    xs_in_region.extend([gmin_x, gmax_x])

        # NC state: same approach
        if len(nc_state_gdf) > 0:
            for _, row in nc_state_gdf.iterrows():
                gmin_x, gmin_y, gmax_x, gmax_y = row.geometry.bounds
                if gmax_y >= y_lo and gmin_y <= y_hi:
                    xs_in_region.extend([gmin_x, gmax_x])

        # CoastSat points
        if len(coastsat_prepped) > 0:
            m = ((coastsat_prepped["y"] >= y_lo) &
                 (coastsat_prepped["y"] <= y_hi))
            if m.any():
                xs_in_region.extend(coastsat_prepped.loc[m, "x"].tolist())

        if xs_in_region:
            x_lo = min(xs_in_region)
            x_hi = max(xs_in_region)
            # Small buffer on both sides + extra room on right for labels
            x_pad_l = (x_hi - x_lo) * 0.15
            x_pad_r = (x_hi - x_lo) * 0.40
            x_lo -= x_pad_l
            x_hi += x_pad_r
        else:
            x_lo, x_hi = 449_000, 462_000  # fallback

        # Slight y padding for visual breathing room
        ...
        if len(nc_state_gdf) > 0:
            for yr in sorted(nc_state_gdf["_year"].unique()):
                ...
                subset.plot(ax=ax, color=year_color(yr),
                            linewidth=1.4, linestyle="--",
                            alpha=0.85, zorder=3, aspect=None)

        # CoastSat (middle layer, dotted)
        if len(coastsat_prepped) > 0:
            for date, grp in sorted(coastsat_prepped.groupby("date"),
                                     key=lambda x: x[0]):
                year = pd.Timestamp(date).year
                grp_sorted = grp.sort_values("alongshore_m")
                ax.plot(grp_sorted["x"], grp_sorted["y"],
        ...
        # Wet-dry (top layer, solid, thickest)
        if len(wet_dry_gdf) > 0:
            for yr in sorted(wet_dry_gdf["_year"].unique()):
                subset = wet_dry_gdf[wet_dry_gdf["_year"] == yr]
                subset.plot(ax=ax, color=year_color(yr),
        ...
        # Location labels within this region's y-range
        for name, y in PLACE_LABELS:
            if ymin <= y <= ymax:
                # Position label near right edge of panel
        ...
        # Small scale bar in each panel (lower-left)
        _draw_scale_bar(ax, x_lo, ymin, x_hi, ymax, length_km=2)

    # ─── Source legend on first panel (top-left) ──────────────────────────
    source_legend = [
        Line2D([0], [0], color="#333", lw=2.6, linestyle="-",
               label="Aerial wet-dry"),
        Line2D([0], [0], color="#333", lw=1.4, linestyle="--",
               label="NC state"),
        Line2D([0], [0], color="#333", lw=1.1, linestyle=":",
               label="CoastSat"),
    ]
    panel_axes[0].legend(handles=source_legend, loc="upper left",
    ...
    cbar.set_ticklabels([str(unique_years[i]) for i in tick_ranks])
    cbar.ax.tick_params(labelsize=8, length=3, color="#666")
    cbar.set_label(f"Year  ({n_years} unique)", fontsize=9, labelpad=6)

    fig.suptitle("Shoreline data — zoomed regional views",
                 fontsize=13, fontweight="bold", y=0.98)
...
def main():
    ...
    if MAKE_VISUALIZATIONS:
        ...
        print("\nGenerating zoomed regional detail view...")
        zoomed_out = os.path.join(OUTPUT_DIR,
                                    "shoreline_inventory_map_zoomed.png")
        plot_shoreline_map_zoomed_regions(
            wet_dry_gdf, nc_state_gdf,
            coastsat_recon, coastsat_alongshore,
            SHORELINE_COLORMAP, BASEMAP, zoomed_out,
        )
