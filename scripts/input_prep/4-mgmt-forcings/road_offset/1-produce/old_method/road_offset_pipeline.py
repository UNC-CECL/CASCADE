...
This script reads raw road and dune raw_offset CSVs (exported from ArcGIS),
...
# Overridable from the shell so both vintages run from one file:
#     HAT_ROAD_YEAR=2004 python road_offset_pipeline.py
YEAR = int(os.environ.get("HAT_ROAD_YEAR", 1984))

# Derived from this file's own location, never hardcoded. OUTPUT_DIR had been
# reduced to "/data/hatteras_init/..." -- an absolute path resolving to the root
# of the current drive, so a run would have written the forcing somewhere no
...
RAW_ROAD_CSV = (DATA / "4-mgmt-forcing" / "road_offset" / "raw_offset"
                / str(YEAR) / f"nc12_{YEAR}.csv")
RAW_DUNE_CSV = (DATA / "2-brie-offset" / "raw_offsets"
                / f"{YEAR}_duneline_offset_raw.csv")

OUTPUT_DIR = (DATA / "4-mgmt-forcing" / "road_offset" / "old_method_offset"
              / str(YEAR))
