...
Clip & Resample Domains tool, so HAT_road_setback_extract.py can measure the
NC-12 setback along the same profiles that define the interior.
...
means comparing two directories:
  ...
  data\hatteras_init\roads\raster\1984\
      RUN_MANIFEST.txt                        every setting that made this folder
      masks\
          domain_9_road_1984.npy              <- HAT_road_setback_extract reads these
...
# --- ROAD ---------------------------------------------------------------
ROAD_YEAR = 1984
ROAD_GEOJSON = Path(r"/data/hatteras_init/roads/raw_offset/1984/nc12_1984.geojson")

# --- OUTPUT LAYOUT ------------------------------------------------------
OUT_ROOT = INIT_ROOT / "roads" / "raster" / str(ROAD_YEAR)
MASK_DIR = OUT_ROOT / "masks"          # <- point HAT_road_setback_extract here
...
QC_DOMAINS = [9, 11, 13, 15, 16, 22, 35, 74, 78, 79, 84, 85, 86]
...
def write_manifest(recs, path: Path):
    ...
    lines = [
        ...
        "=" * 78, "",
        "CONTENTS",
        r"  masks\                    road masks -> HAT_road_setback_extract.py",
...
def main():
    ...
    print(f"\n  Next: HAT_road_setback_extract.py")
    print(f"    ROAD_MASK_DIR = r\"{MASK_DIR}\"")
