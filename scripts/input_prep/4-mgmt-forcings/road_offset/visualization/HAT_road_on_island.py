...
DUNE_OFFSET_FILE = os.path.join(
    ...
    f"Island_Dune_Offsets_{CHECK_YEAR}_CASCADE_Input_unpadded.csv",
)
DUNE_OFFSET_COLUMN = 0     # 0 if single-column; set if the file has year columns

# NOTE: file naming changed from "..._PADDED_120.csv" (120 rows: 15 buffer +
# 90 real + 15 buffer) to "..._CASCADE_Input_unpadded.csv". This is assumed to
# mean the file now holds exactly NUM_REAL_DOMAINS (90) rows, one per real
# domain starting at FIRST_FILE_NUMBER, with NO buffer rows. If that's wrong
# (e.g. it's still 120 rows despite the name, or starts at a different domain),
# the offs[] lookup below will silently misalign -- flag it if the printed
# offset range in the console comparison looks off.
DUNE_OFFSET_IS_PADDED = False
...
C_PIER   = "#1565C0"
C_GROIN  = "#B71C1C"
C_RELOC  = "#FF3B30"    # bright red for relocated road location, distinct
                        # from the dark maroon-red NC-12 footprint (C_GROIN)
C_INK    = "#1a1a2e"
...
def load_offsets():
    """The file has a header row: Domain_ID, {year}. Select the year column
    by name rather than a fixed index -- a hardcoded DUNE_OFFSET_COLUMN=0
    was picking up Domain_ID (1-90) instead of the real offset values,
    producing near-zero alongshore shifts instead of the true curving
    planform."""
    df = pd.read_csv(DUNE_OFFSET_FILE)
    col = str(CHECK_YEAR)
    if col in df.columns:
        vals = df[col].to_numpy(dtype=float)
    else:
        vals = df.iloc[:, DUNE_OFFSET_COLUMN].to_numpy(dtype=float)
        print(f"  !! Column '{col}' not found in {DUNE_OFFSET_FILE} "
              f"(columns: {list(df.columns)}); falling back to column index "
              f"{DUNE_OFFSET_COLUMN} ('{df.columns[DUNE_OFFSET_COLUMN]}') -- "
              f"verify this is the offset value, not Domain_ID")
    return np.round(vals / DAM_TO_M).astype(int)     # m -> cells


def load_setbacks():
    """Matches SECTION 9 of the run script: skiprows=1 (header row present),
    single positional column, one value per road domain FIRST_ROAD_DOMAIN..
    LAST_ROAD_DOMAIN. The run script zero-pads any shortfall when it builds
    road_setbacks_full; here we leave short domains unset (None) rather than
    assuming 0, so a missing setback isn't mistaken for a real "at row 0"
    value in the plot."""
    raw = np.loadtxt(ROAD_SETBACK_FILE, skiprows=1, delimiter=",")
    vals = np.atleast_1d(raw.squeeze())
    gis_ids = list(range(FIRST_ROAD_DOMAIN, LAST_ROAD_DOMAIN + 1))
    if vals.size != len(gis_ids):
        print(f"  !! setback file has {vals.size} values after header row, "
              f"expected {len(gis_ids)} (GIS {FIRST_ROAD_DOMAIN}-"
              f"{LAST_ROAD_DOMAIN}); mapping positionally up to the shorter "
              f"length, remaining domains left unset")
...
def main():
    ...
    print(f"\n  Loaded dune offsets: {len(offsets_all)} rows "
          f"(expected {NUM_REAL_DOMAINS} if unpadded, {TOTAL_DOMAINS} if "
          f"padded; DUNE_OFFSET_IS_PADDED = {DUNE_OFFSET_IS_PADDED})")
    ...
    for g in range(FIRST_FILE_NUMBER, FIRST_FILE_NUMBER + NUM_REAL_DOMAINS):
        ...
        if DUNE_OFFSET_IS_PADDED:
            pad = START_REAL_INDEX + (g - FIRST_FILE_NUMBER)
        else:
            pad = g - FIRST_FILE_NUMBER
    ...
    def draw_road(a, zoom=False):
        ...
        for year, per in HISTORICAL_RELOCATIONS.items():
            ...
            for g, sb in sorted(per.items()):
                ...
                a.add_patch(mpatches.Rectangle(
                    (c0, off + r0), c1 - c0, max(r1 - r0, 1),
                    facecolor=C_RELOC, edgecolor="none",
                    alpha=0.95 if zoom else 0.85, zorder=7))
    ...
    d["relocated road location"] = mpatches.Patch(color=C_RELOC)
