...
OUTPUT_ROOT_DIR = "shoreline_duneline_compare"
...
# --- Shoreline DSAS (observed shoreline change) ---
...
# --- Dune-line change (observed dune movement, per-domain) ---
DUNE_DOMAIN_CSV = r"C:\Users\hanna\PycharmProjects\CASCADE\data\hatteras_init\duneline_change\dune_change_1978_1997_by_domain.csv"
DUNE_DOMAIN_COL = "domain_id"            # GIS domain ID in dune file (1–90)
DUNE_CHANGE_COL = "dune_change_m"        # dune-line change column (m)
# ============================================================


def extract_run_name(npz_path):
    """Return filename without extension, e.g., 'HAT_1978_1997_Natural_State'."""
...
def plot_total_shoreline_change_over_run(
    ...
    obs_shore_domains=None,
    obs_shore_change=None,
    obs_dune_domains=None,
    obs_dune_change=None,
):
    """
    ...
    Overlays:
    - Model shoreline change (CASCADE)
    - Observed shoreline change (DSAS)
    - Observed dune-line change
    """
    os.makedirs(outdir, exist_ok=True)
    ...
    ax.axhline(0, linestyle="--", linewidth=1, alpha=0.7, color="#666666")

    # Shade buffer regions with light blue
    buffer_color = "#c6dbef"  # soft blue
    if left_buffer_end is not None and left_buffer_end >= 0:
        ax.axvspan(
            domain_numbers[0] - 0.5,
            left_buffer_end + 0.5,
            alpha=0.3,
            color=buffer_color,
        )
    if right_buffer_start is not None and right_buffer_start < ndom:
        ax.axvspan(
            right_buffer_start - 0.5,
            domain_numbers[-1] + 0.5,
            alpha=0.3,
            color=buffer_color,
        )

    # Observed shoreline DSAS change: black
    if obs_shore_domains is not None and obs_shore_change is not None:
        ax.plot(
            obs_shore_domains,
            obs_shore_change,
            marker="x",
            linewidth=1.2,
            color="#000000",
            label="Observed shoreline",
        )

    # Observed dune-line change: orange
    if obs_dune_domains is not None and obs_dune_change is not None:
        ax.plot(
            obs_dune_domains,
            obs_dune_change,
            marker="^",
    ...
    # Labels and title
    ax.set_ylabel(f"Total change over run ({units})")
    ax.set_title(f"{run_name} – Total Shoreline & Dune Change Over Simulation")
    ...
    if left_buffer_end is not None and right_buffer_start is not None:
        ...
        ax.text(
            (domain_numbers[0] + left_buffer_end) / 2,
            y_top * 0.9,
            "Left buffer",
            ha="center",
            va="top",
        ...
        ax.text(
            (right_buffer_start + domain_numbers[-1]) / 2,
            y_top * 0.9,
            "Right buffer",
            ha="center",
            va="top",
    ...
    if (obs_shore_domains is not None and obs_shore_change is not None) or \
       (obs_dune_domains is not None and obs_dune_change is not None):
        ax.legend()
...
def main():
    ...
    # ----- Observed shoreline DSAS change -----
    obs_shore_domains = None
    obs_shore_change = None
    if DSAS_DOMAIN_CSV is not None:
        ...
        obs_shore_domains = obs_domains_all[mask]
        obs_shore_change = obs_change_all[mask]

    # ----- Observed dune-line change (already per-domain from prep script) -----
    obs_dune_domains = None
    obs_dune_change = None
    if DUNE_DOMAIN_CSV is not None:
        dune_df = pd.read_csv(DUNE_DOMAIN_CSV)

        gis_dune_domains = dune_df[DUNE_DOMAIN_COL].to_numpy()
        dune_change_all = dune_df[DUNE_CHANGE_COL].to_numpy()

        dune_domains_all = gis_dune_domains + LEFT_BUFFER_END

        mask_dune = (dune_domains_all > LEFT_BUFFER_END) & (dune_domains_all < RIGHT_BUFFER_START)
        obs_dune_domains = dune_domains_all[mask_dune]
        obs_dune_change = dune_change_all[mask_dune]
    ...
    # 3) Total shoreline change over run per domain, with shoreline + dune overlays
    plot_total_shoreline_change_over_run(
        ...
        obs_shore_domains=obs_shore_domains,
        obs_shore_change=obs_shore_change,
        obs_dune_domains=obs_dune_domains,
        obs_dune_change=obs_dune_change,
    )
