...
# with a freehand crest-reference band, two real dune rows controlled by the
# user-drawn interior boundary, local interior alignment,
...
#
#   In the partition QC, an interior start at cell s extracts dune rows from
#   cells s-DUNE_ROWS through s-1, then starts interior row 0 at cell s.
#
...
VERSION = "Roya_1996_cross_ROAD_2row_interior_controlled_real_cells"
DEM_YEAR = "2011"
...
MIN_DUNE_H_M = 0.1         # m, Barrier3D minimum dune height above berm
...
# Apply the already-verified standalone correction ONLY to the final CASCADE
# arrays written to disk. The picker, road masks, saved search windows, and QC
# geometry remain in their existing extraction orientation.
CORRECT_SAVED_ALONGSHORE = True
...
DUNE_ROWS = 2              # Number of real 10 m cross-shore dune cells.
                           ...
                           # For Pea Island this is (50, 2). If the chosen
                           # interior starts at cell s, row 0 uses s-2 and row 1
                           # uses s-1.
FOLLOW_LOCAL_DUNE_LINE = True
                           # Each alongshore profile is re-indexed independently:
                           #   dune rows = DUNE_ROWS cells immediately seaward
                           #   interior row 0 = the cell on the chosen line
                           # This makes the rectangular output follow the curved dune.
# REAL-SOURCE-CELL POLICY:
#   * never interpolate a missing dune location from neighboring profiles;
#   * never turn water/NoData into a dune cell;
#   * block acceptance/saving until every profile supplies DUNE_ROWS consecutive
#     real DEM cells immediately seaward of the chosen interior line;
#   * allow Barrier3D's configured minimum dune height for real selected cells
#     below the model berm. This changes the model height, never the selected
#     source-cell location, and is reported in the QC text/console.
FILL_MISSING_DUNE = False
REQUIRE_REAL_DUNE_CELLS = True
ALLOW_MIN_DUNE_HEIGHT_FLOOR = True
TRIM_INTERIOR_ROWS = False # Preserve all TOPO_ROWS = 200 rows, including
                           # back-barrier/landward sentinel water at -3 m MHW.

# Plot colors identify the exact source-GIS cells that become Barrier3D dune
# rows. Their locations are determined by the chosen interior-start line.
DUNE_ROW_COLORS = ("#6A1B9A", "#00897B")
...
def reverse_saved_topography_alongshore(array: np.ndarray) -> np.ndarray:
    """Reverse only the alongshore axis of saved (cross_shore, alongshore) topo."""
    array = np.asarray(array)
    expected_shape = (TOPO_ROWS, ALONG_COLS)
    if array.shape != expected_shape:
        raise ValueError(
            f"Expected saved topography shape {expected_shape}, got {array.shape}"
        )
    return np.ascontiguousarray(array[:, ::-1])


def reverse_saved_packed_dune_alongshore(array: np.ndarray) -> np.ndarray:
    """Reverse alongshore within every packed dune row without swapping rows."""
    array = np.asarray(array)
    expected_shape = (DUNE_ROWS * ALONG_COLS,)
    if array.shape != expected_shape:
        raise ValueError(
            f"Expected packed dune shape {expected_shape}, got {array.shape}"
        )
    dune_rows = array.reshape(DUNE_ROWS, ALONG_COLS)
    return np.ascontiguousarray(dune_rows[:, ::-1].reshape(-1))
...
def normalize_interior_start_line(
    ...
    # The interior boundary also determines the DUNE_ROWS immediately seaward
    # of it, so it cannot be placed closer to the ocean than DUNE_ROWS.
    maximum = max(0, n_cross - 1)
    minimum = min(DUNE_ROWS, maximum)
    return np.clip(line, minimum, maximum)


def extracted_dune_cell_info(
        profile: np.ndarray,
        used_crest_cell: int,
        berm_mhw: float,
        ) -> list[dict]:
    """Describe the exact real source cells proposed for the saved dune rows.

    This function reports invalid cells as invalid. It never converts an invalid
    source cell to ``MIN_DUNE_H_M``.
    """
    profile = np.asarray(profile, dtype=float)
    rows = []
    for q in range(DUNE_ROWS):
        # When no crest was detected, every requested dune row is unresolved.
        # Do not let q=1 accidentally turn a missing crest (-1) into source cell 0.
        cell = int(used_crest_cell + q) if used_crest_cell >= 0 else -1
        valid = (
            0 <= cell < profile.size
            and np.isfinite(profile[cell])
            and profile[cell] > SENTINEL_WATER_M + 1e-9
        )
        elevation = float(profile[cell]) if valid else np.nan
        raw_height = elevation - berm_mhw if valid else np.nan
        would_require_floor = bool(
            valid and raw_height < MIN_DUNE_H_M
        )
        height = (
            max(raw_height, MIN_DUNE_H_M)
            if valid and ALLOW_MIN_DUNE_HEIGHT_FLOOR
            else raw_height
        )
        rows.append({
            "row": q,
            "cell": cell,
            "elevation_mhw": elevation,
            "elevation_navd88": elevation + MHW_M if valid else np.nan,
            "height_above_berm": height,
            "would_require_floor": would_require_floor,
            "valid": bool(valid),
        })
    return rows


def real_dune_cell_issues(
        prof_arr: np.ndarray,
        interior_start_line: np.ndarray,
        n_fill: int | None = None,
        ) -> list[dict]:
    """Return every condition that would require invented dune input data.

    The user-selected interior line controls the partition. For an interior start
    ``s``, dune row 0 comes from ``s - DUNE_ROWS`` and the last dune row comes
    from ``s - 1``. A profile passes only when those cells are finite, non-water
    DEM cells. In fully strict mode, each elevation must also yield at least
    ``MIN_DUNE_H_M`` above the configured berm; otherwise the old code would
    silently raise it to the model floor.
    """
    prof_arr = np.asarray(prof_arr, dtype=float)
    interior_start_line = np.asarray(interior_start_line, dtype=int)
    n_along, n_cross = prof_arr.shape
    if n_fill is None:
        n_fill = min(ALONG_COLS, n_along)
    n_fill = min(int(n_fill), n_along, interior_start_line.size)
    berm_mhw = BERM_ELEV_NAVD_M - MHW_M
    issues = []

    for i in range(n_fill):
        start = int(interior_start_line[i])
        first_dune_cell = start - DUNE_ROWS
        if first_dune_cell < 0:
            issues.append({
                "profile": i,
                "row": None,
                "cell": first_dune_cell,
                "reason": (
                    f"interior starts at cell {start}, leaving fewer than "
                    f"{DUNE_ROWS} seaward dune cells"
                ),
            })
            continue

        for q in range(DUNE_ROWS):
            src = first_dune_cell + q
            if src < 0 or src >= n_cross:
                issues.append({
                    "profile": i,
                    "row": q,
                    "cell": src,
                    "reason": "source cell lies outside the DEM",
                })
                continue
            ...
            if not np.isfinite(z) or z <= SENTINEL_WATER_M + 1e-9:
                issues.append({
                    ...
                    "row": q,
                    "cell": src,
                    "reason": "source cell is water/NoData",
                })
                continue

            raw_h = z - berm_mhw
            if (
                not ALLOW_MIN_DUNE_HEIGHT_FLOOR
                and raw_h < MIN_DUNE_H_M
            ):
                issues.append({
                    "profile": i,
                    "row": q,
                    "cell": src,
                    "reason": (
                        f"real z={z:.3f} m MHW gives h={raw_h:.3f} m "
                        f"above berm, below the {MIN_DUNE_H_M:.3f} m model "
                        "minimum"
                    ),
                })
...
def format_real_dune_issues(
        stem: str,
        issues: list[dict],
        max_lines: int = 20,
        ) -> str:
    """Create a concise, actionable strict-validation message."""
    lines = []
    for issue in issues[:max_lines]:
        ...
        if row is not None:
            where += f", dune row {row}"
        if cell is not None:
            where += f", source cell {cell}"
        lines.append(f"  - {where}: {issue['reason']}")

    remaining = len(issues) - len(lines)
    if remaining > 0:
        lines.append(f"  - ... plus {remaining} additional issue(s)")

    return (
        f"{stem}: STRICT REAL-DATA CHECK FAILED\n"
        + "\n".join(lines)
        + "\nNo dune or topography arrays were written for this domain.\n"
        + "Move the interior line so the immediately seaward dune cells are "
          "real and valid. Press P only if you also want to redraw the visual "
          "crest-reference band, or S to skip the domain."
    )
...
def pre_extraction_cross_section_qc(
    ...
    # Detected crests provide the initial/default interior line and a visual
    # reference only. The user's interior line controls the extracted cells.
    ...
    fig.text(
        ...
        "←/→ profile   U undo   R reset   P repick dune band   "
        "Enter validate/accept   S skip   Q quit",
        ha="center", va="top",
        fontsize=9.2,
    )
    ...
    ax_map.plot(
        ...
        color="black", lw=1.0, ls="--", marker=".", ms=3,
        label="detected crest (reference only)",
    )
    dune_row_artists = []
    for q in range(DUNE_ROWS):
        source_line = state["interior"] - DUNE_ROWS + q
        artist, = ax_map.plot(
            x_along,
            source_line,
            color=DUNE_ROW_COLORS[q % len(DUNE_ROW_COLORS)],
            lw=1.5, marker=".", ms=4,
            label=f"dune row {q}: chosen from interior line",
        )
        dune_row_artists.append(artist)
    ...
    def active_default_start(profile_index: int) -> int:
        """Default reference based on the detected crest, when available."""
        loc = int(dune_loc_used[profile_index])
        return max(DUNE_ROWS, loc + DUNE_ROWS) if loc >= 0 else DUNE_ROWS
    ...
    def set_active_interior_from_distance(distance_m: float):
        ...
        cell = int(np.clip(cell, DUNE_ROWS, n_cross - 1))
    ...
    def draw_profile_axis(ax, zoom: bool):
        ...
        used = chosen - DUNE_ROWS
        dune_cells = extracted_dune_cell_info(profile, used, berm_mhw)
        ...
        if used != detected and 0 <= used < n_cross:
            ax.scatter(
                ...
                label=f"selected dune row 0 cell {used}",
            )

        # Highlight every exact source-GIS cell that will be extracted into the
        # Barrier3D dune field. Cell spans use half-cell edges so the colored
        # regions correspond to the actual 10 m raster cells.
        for info in dune_cells:
            q = int(info["row"])
            cell = int(info["cell"])
            color = DUNE_ROW_COLORS[q % len(DUNE_ROW_COLORS)]
            if not (0 <= cell < n_cross):
                continue
            ax.axvspan(
                (cell - 0.5) * CELL_SIZE_M,
                (cell + 0.5) * CELL_SIZE_M,
                color=color, alpha=0.16, zorder=2,
                label=f"EXTRACT dune row {q}: source cell {cell}",
            )
            if info["valid"]:
                ax.scatter(
                    cell * CELL_SIZE_M,
                    info["elevation_mhw"],
                    s=55, marker="s", facecolor=color,
        ...
        if zoom:
            focus = [
                drawn_start, drawn_end, effective_start, used, chosen,
                *[int(info["cell"]) for info in dune_cells],
            ]
            ...
            saved_profile = ALONG_COLS - 1 - i if CORRECT_SAVED_ALONGSHORE else i
            row_lines = []
            for info in dune_cells:
                q = int(info["row"])
                cell = int(info["cell"])
                if info["valid"]:
                    floor_note = (
                        " | model floor applied"
                        if info["would_require_floor"]
                        and ALLOW_MIN_DUNE_HEIGHT_FLOOR else ""
                    )
                    row_lines.append(
                        f"Dune row {q}: source cell {cell} | "
                        f"z={info['elevation_mhw']:.2f} m MHW | "
                        f"h={info['height_above_berm']:.2f} m above berm"
                        f"{floor_note}"
                    )
                else:
                    row_lines.append(
                        f"Dune row {q}: source cell {cell} | INVALID — "
                        "acceptance blocked"
                    )
            dune_info_text = (
                f"EXACT DUNE CELLS TO EXTRACT\n"
                f"Displayed profile {i} -> saved alongshore profile {saved_profile}\n"
                + "\n".join(row_lines)
                + f"\nDefault interior begins at source cell {used + DUNE_ROWS}"
            )
            ax.text(
                0.015, 0.98, dune_info_text,
    ...
    def redraw():
        ...
        for q, artist in enumerate(dune_row_artists):
            artist.set_data(
                x_along,
                state["interior"] - DUNE_ROWS + q,
            )
        ...
        i = state["active"]
        used = int(state["interior"][i]) - DUNE_ROWS
        active_road = road_profile_summary(
            road_mask, i, used
        )
        dune_status = (
            f"dune rows = selected source cells "
            f"{used}–{used + DUNE_ROWS - 1}"
        )
        status_artist.set_text(
            f"{fig_title(stem)}   |   active profile {i}   |   "
            f"{dune_status}   |   interior cell {int(state['interior'][i])}"
        )
        if state["validation_error"]:
            road_status_artist.set_color("#B71C1C")
            road_status_artist.set_text(state["validation_error"])
        else:
            road_status_artist.set_color("black")
    ...
    def on_key(event):
        ...
        if key in ("enter", "return", "a"):
            issues = real_dune_cell_issues(
                prof_arr, state["interior"], n_fill
            )
            if REQUIRE_REAL_DUNE_CELLS and issues:
                first_profile = int(issues[0]["profile"])
                state["active"] = first_profile
                state["validation_error"] = (
                    f"STRICT CHECK: {len(issues)} issue(s); profile "
                    f"{first_profile}: {issues[0]['reason']}. Move the interior line."
                )
                print(format_real_dune_issues(stem, issues))
                redraw()
                return
            state["action"] = "accept"
            plt.close(fig)
    ...
    used_loc = int(final_line[active]) - DUNE_ROWS
    used_elev = (
...
def build_local_dunes_and_interior(
    ...
    topo_m : ndarray, shape (TOPO_ROWS, ALONG_COLS)
        ...
        source-GIS cell for each profile.
    dune_loc_used : ndarray, shape (n_along,)
        Source-GIS location assigned to dune row 0. It is exactly
        ``interior_start - DUNE_ROWS`` for every profile.
    ...
    chosen_interior_start = normalize_interior_start_line(
        ...
        np.asarray(dune_loc, dtype=int),
        n_along,
        n_cross,
    )
    dune_loc_used = chosen_interior_start - DUNE_ROWS

    issues = real_dune_cell_issues(
        prof_arr, chosen_interior_start, n_fill
    )
    if REQUIRE_REAL_DUNE_CELLS and issues:
        raise ValueError(format_real_dune_issues(stem, issues))
    ...
    for i in range(n_fill):
        ...
        # Their locations are controlled entirely by the chosen interior line:
        # for two rows, interior cell 13 means dune cells 11 and 12.
        for q in range(DUNE_ROWS):
            src = loc + q
            if src < 0 or src >= n_cross:
                raise ValueError(
                    f"profile {i}, dune row {q}, source cell {src}: "
                    "outside the DEM"
                )
            z = float(prof_arr[i, src])
            if not np.isfinite(z) or z <= SENTINEL_WATER_M + 1e-9:
                raise ValueError(
                    f"profile {i}, dune row {q}, source cell {src}: "
                    "not a real finite DEM land cell"
                )

            raw_h = z - berm_mhw
            if raw_h < MIN_DUNE_H_M and not ALLOW_MIN_DUNE_HEIGHT_FLOOR:
                raise ValueError(
                    f"profile {i}, dune row {q}, source cell {src}: "
                    f"real height {raw_h:.3f} m above berm is below the "
                    f"Barrier3D minimum {MIN_DUNE_H_M:.3f} m"
                )
            h = max(raw_h, MIN_DUNE_H_M) if (
                ALLOW_MIN_DUNE_HEIGHT_FLOOR
            ) else raw_h
            dune_m[i, q] = h

        # Interior row 0 begins exactly at the user's boundary.
...
def extract_domain(stem, prof_arr, start_beach, i0_line, i1_line,
    ...
    if missing.any():
        ...
        print(
            f"       [reference only] {stem}: no detected crest at profiles "
            f"{idx}; chosen interior lines still control dune extraction"
        )
    ...
    # Report model-floor use without treating it as a fake/missing source cell.
    # Every counted value still came from the exact real DEM cell selected by the
    # user's interior boundary.
    berm_mhw = BERM_ELEV_NAVD_M - MHW_M
    n_height_floored = 0
    for i in range(n_fill):
        first = int(interior_start_line[i]) - DUNE_ROWS
        for q in range(DUNE_ROWS):
            z = float(prof_arr[i, first + q])
            if z - berm_mhw < MIN_DUNE_H_M:
                n_height_floored += 1
    if ALLOW_MIN_DUNE_HEIGHT_FLOOR and n_height_floored:
        print(
            f"       [model floor] {stem}: {n_height_floored}/"
            f"{n_fill * DUNE_ROWS} real selected dune-cell heights raised to "
            f"{MIN_DUNE_H_M:.2f} m above the configured berm"
        )
    ...
    dune_dm_packed = dune_dm_2d.T.reshape(-1)

    expected_saved_shape = (ALONG_COLS * DUNE_ROWS,)
    if dune_dm_packed.shape != expected_saved_shape:
        raise ValueError(
            f"{stem}: packed dune array shape {dune_dm_packed.shape} does not "
            f"match expected {expected_saved_shape}"
        )

    # Safety check: unpack the packed representation and verify that it exactly
    # reconstructs the internal (alongshore, dune-row) array.
    dune_check = dune_dm_packed.reshape(DUNE_ROWS, ALONG_COLS).T
    ...
    # Apply exactly the same alongshore correction as the verified standalone
    # correction script, but only to the final arrays written for CASCADE.
    if CORRECT_SAVED_ALONGSHORE:
        topo_dm_save = reverse_saved_topography_alongshore(topo_dm)
        dune_dm_save = reverse_saved_packed_dune_alongshore(dune_dm_packed)

        if not np.array_equal(topo_dm_save, topo_dm[:, ::-1], equal_nan=True):
            raise RuntimeError(f"{stem}: saved topography reversal failed")

        expected_dune_save = (
            dune_dm_packed.reshape(DUNE_ROWS, ALONG_COLS)[:, ::-1].reshape(-1)
        )
        if not np.array_equal(dune_dm_save, expected_dune_save, equal_nan=True):
            raise RuntimeError(f"{stem}: saved packed-dune reversal failed")
    else:
        topo_dm_save = np.ascontiguousarray(topo_dm)
        dune_dm_save = np.ascontiguousarray(dune_dm_packed)

    print(
        f"Two-row dune input: internal={dune_dm_2d.shape}, "
        f"saved={dune_dm_save.shape}, "
        f"alongshore_corrected={CORRECT_SAVED_ALONGSHORE}"
    )

    # Row 0 begins DUNE_ROWS cells seaward of the selected interior boundary;
    # the final dune row is immediately seaward of that boundary.
    ...
    np.save(topo_out, topo_dm_save)
    np.save(dune_out, dune_dm_save)

    # Reload and verify the exact arrays that were written to disk.
    if not np.array_equal(np.load(topo_out), topo_dm_save, equal_nan=True):
        raise RuntimeError(f"{stem}: reloaded topography does not match save array")
    if not np.array_equal(np.load(dune_out), dune_dm_save, equal_nan=True):
        raise RuntimeError(f"{stem}: reloaded dune does not match save array")

    band_desc = describe_window_lines(i0_line, i1_line)
    print(f"[ok] {stem}: {band_desc}, {n_found}/{n_along} reference crests, "
    ...
    return {
        ...
        "n_filled": 0,
        "n_height_floored": int(n_height_floored),
        ...
        "dune_h": crest_h,
        # These remain in extraction/QC orientation. Only the disk arrays above
        # receive the final alongshore correction, matching the old standalone
        # correction workflow exactly.
        "topo_dm": topo_dm, "dune_dm": dune_dm_2d,
        "topo_dm_saved": topo_dm_save,
        "dune_dm_saved": dune_dm_save,
...
def qc_figure(stem, prof_arr, start_beach, res, fig_dir: Path, road_mask=None):
    """QC figure showing reference crests and user-selected source cells."""
    n_along, n_cross = prof_arr.shape
    zm = masked_profiles(prof_arr)
    road_mask = validate_road_mask(road_mask, prof_arr.shape)
    ...
    ax_map.plot(x, dl, color="black", lw=1.0, ls="--", marker=".", ms=3,
                label="detected crest (reference only)")
    dune_used = np.asarray(res["dune_loc_used"], dtype=float)
    for q in range(DUNE_ROWS):
        ax_map.plot(
            x, np.where(dune_used >= 0, dune_used + q, np.nan),
            color=DUNE_ROW_COLORS[q % len(DUNE_ROW_COLORS)],
            lw=1.1, marker=".", ms=4,
            label=f"dune row {q}: selected from interior line",
        )
    ...
    dune_dm = np.asarray(res["dune_dm"], dtype=float)
    dune_x = x[:dune_dm.shape[0]]
    for q in range(min(DUNE_ROWS, dune_dm.shape[1])):
        ax_h.plot(
            dune_x, dune_dm[:, q] * 10.0,
            color=DUNE_ROW_COLORS[q % len(DUNE_ROW_COLORS)], lw=1.5,
            marker="o", ms=3,
            label=(
                f"dune row {q}: crest cell" if q == 0
                else f"dune row {q}: crest+{q} landward cell"
            ),
        )
    ...
    fig.suptitle(
        ...
        f"{res['n_found']}/{n_along} dunes found  |  "
        f"extracting {DUNE_ROWS} source cells/profile: crest through "
        f"crest+{DUNE_ROWS - 1}", fontsize=12
    )
...
def middle_cross_section_figure(stem, prof_arr, start_beach, res, fig_dir: Path, road_mask=None):
    ...
    berm_mhw = BERM_ELEV_NAVD_M - MHW_M
    dune_cells = extracted_dune_cell_info(profile, used_loc, berm_mhw)
    ...
    for info in dune_cells:
        q = int(info["row"])
        res[f"middle_dune_row_{q}_source_cell"] = int(info["cell"])
        res[f"middle_dune_row_{q}_elev_m_mhw"] = info["elevation_mhw"]
        res[f"middle_dune_row_{q}_height_above_berm_m"] = (
            info["height_above_berm"]
        )
    ...
    def add_geometry(ax, include_labels=True):
        ...
        # The selected source cell is controlled by the interior boundary.
        if used_loc != detected_loc and 0 <= used_loc < n_cross:
            ax.scatter(
                used_loc * CELL_SIZE_M, used_elev,
                s=90, marker="D", color="#00897B", zorder=9,
                label=(
                    f"selected dune row 0: cell {used_loc}, "
        ...
        # Show the exact consecutive source cells assigned to dune rows 0..N-1.
        for info in dune_cells:
            q = int(info["row"])
            cell = int(info["cell"])
            color = DUNE_ROW_COLORS[q % len(DUNE_ROW_COLORS)]
            ...
            ax.axvspan(
                ...
                color=color, alpha=0.16, zorder=2,
                label=(
                    f"EXTRACT dune row {q}: source cell {cell}"
                    if include_labels else None
                ),
            )
            if info["valid"]:
    ...
    status = "selected from the interior boundary"
    ...
    saved_profile = ALONG_COLS - 1 - mid if CORRECT_SAVED_ALONGSHORE else mid
    dune_row_text = []
    for info in dune_cells:
        q = int(info["row"])
        cell = int(info["cell"])
        if info["valid"]:
            ...
            dune_row_text.append(
                f"Dune row {q}: source cell {cell}, "
                f"z={info['elevation_mhw']:.2f} m MHW, "
                f"h={info['height_above_berm']:.2f} m{floor_note}"
            )
        else:
            dune_row_text.append(
                f"Dune row {q}: source cell {cell}, INVALID — "
    ...
    analysis_text = (
        f"EXACT DUNE CELLS TO EXTRACT\n"
        f"Displayed profile {mid} -> saved profile {saved_profile}\n"
        + "\n".join(dune_row_text) + "\n"
...
def global_config() -> dict:
    ...
    return {
        ...
        "CORRECT_SAVED_ALONGSHORE": CORRECT_SAVED_ALONGSHORE,
...
def main():
    ...
    if MODE in ("run", "pick_and_run"):
        ...
        for name in names:
            ...
            # build_local_dunes_and_interior() is called. Press P to redraw
            # the freehand crest-reference band and return to this QC view.
            skip_domain = False
            stop_run = False
            qc_info = {}
